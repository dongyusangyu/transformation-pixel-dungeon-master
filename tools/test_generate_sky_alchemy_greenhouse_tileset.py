...
GENERATOR_PATH = ROOT / "tools" / "generate_sky_alchemy_greenhouse_tileset.py"
SEWERS_PATH = ENV / "tiles_sewers.png"
ATLAS_PATH = ENV / "tiles_sky_alchemy_greenhouse.png"
WATER_PATH = ENV / "water_sky_alchemy_greenhouse.png"
...
def load_generator():
    spec = importlib.util.spec_from_file_location("sky_greenhouse_generator", GENERATOR_PATH)
...
def green_pixels(image: Image.Image) -> int:
    return sum(1 for red, green, blue, alpha in image.getdata()
               if alpha and green >= red + 12 and green >= blue + 4)


def cyan_pixels(image: Image.Image) -> int:
    return sum(1 for red, green, blue, alpha in image.getdata()
               if alpha and blue >= red + 15 and green >= red + 10)


def warm_pixels(image: Image.Image) -> int:
    return sum(1 for red, green, blue, alpha in image.getdata()
               if alpha and red >= blue + 20 and green >= blue + 8 and red >= 80)


class SkyAlchemyGreenhouseContractTest(unittest.TestCase):
    ...
    def test_saved_assets_match_the_current_generator(self) -> None:
        generator = load_generator()
...
class SkyAlchemyGreenhouseSemanticTest(unittest.TestCase):
    def setUp(self) -> None:
        self.atlas = load_generator().build_tileset()

    def test_floors_are_readable_distinct_and_botanical(self) -> None:
        variants = []
        for index in (FLOOR, FLOOR_ALT_1, FLOOR_ALT_2):
            gray = tile(self.atlas, index).convert("L")
            stats = ImageStat.Stat(gray)
            self.assertGreaterEqual(stats.mean[0], 45)
            self.assertGreaterEqual(stats.stddev[0], 3)
            variants.append(digest(gray))
    ...
    def test_tall_live_and_withered_herbs_have_distinct_semantics(self) -> None:
        live = tile(self.atlas, FLAT_HIGH_GRASS)
        dry = tile(self.atlas, FLAT_FURROWED)
        self.assertGreaterEqual(green_pixels(live), 20)
        self.assertGreaterEqual(warm_pixels(dry), 15)
        self.assertNotEqual(digest(live), digest(dry))

        for base, overhang in ((RAISED_HIGH_GRASS, HIGH_GRASS_OVERHANG),
                               (RAISED_FURROWED, FURROWED_OVERHANG)):
            self.assertIsNotNone(tile(self.atlas, base).getchannel("A").getbbox())
            self.assertIsNotNone(tile(self.atlas, overhang).getchannel("A").getbbox())

    def test_raised_herb_stems_align_with_their_overhangs(self) -> None:
        for base, overhang in ((RAISED_HIGH_GRASS, HIGH_GRASS_OVERHANG),
                               (RAISED_FURROWED, FURROWED_OVERHANG)):
    ...
    def test_environment_props_are_visually_distinct(self) -> None:
    ...
    def test_spores_are_not_plain_floor(self) -> None:
        self.assertNotEqual(digest(tile(self.atlas, EMBERS)), digest(tile(self.atlas, FLOOR)))
        self.assertGreaterEqual(green_pixels(tile(self.atlas, EMBERS)), 3)


class SkyAlchemyGreenhouseCompatibilityTest(unittest.TestCase):
    def setUp(self) -> None:
        self.atlas = load_generator().build_tileset()
