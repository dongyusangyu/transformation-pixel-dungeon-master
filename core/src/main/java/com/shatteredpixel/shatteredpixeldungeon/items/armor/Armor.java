...
public class Armor extends EquipableItem {
	...
	public boolean doEquip( Hero hero ) {
		if(hero.heroClass == HeroClass.SLIMEGIRL && !((this instanceof SlimeArmor ) || (this instanceof  SlimeGirlArmor))){
			GLog.n( Messages.get(Armor.class, "slime") );
			return false;
		}
	...
	public int DRMax(int lvl){
		if (Dungeon.isChallenged(Challenges.NO_ARMOR)){
			if(hero!=null){
				return 1 + tier + lvl + augment.defenseFactor(lvl)+hero.pointsInTalent(Talent.HEAVY_APPLICATION)*tier+hero.pointsInTalent(Talent.THICKENED_ARMOR);
			}else{
		...
		if (lvl > max){
			if(hero!=null){
				return ((lvl - max)+1)/2+hero.pointsInTalent(Talent.HEAVY_APPLICATION)*tier+hero.pointsInTalent(Talent.THICKENED_ARMOR);
			}else{
				return ((lvl - max)+1)/2;
			}
			...
			if(hero!=null){
				return max+hero.pointsInTalent(Talent.HEAVY_APPLICATION)*tier+hero.pointsInTalent(Talent.THICKENED_ARMOR);
			}else{
				return max;
			}
	...
	public int DRMin(int lvl){
		if (Dungeon.isChallenged(Challenges.NO_ARMOR)){
			if(hero !=null){
				return hero.pointsInTalent(Talent.LIQUID_ARMOR);
			}
			return 0;
		}
		if (hero != null){//史莱姆娘 防止日志界面崩溃
			lvl += hero.pointsInTalent(Talent.LIQUID_ARMOR);
		}
	...
	public float evasionFactor( Char owner, float evasion ){
		...
		if (owner instanceof Hero){
			...
			if(Dungeon.isChallenged(Challenges.NO_ARMOR) && Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT)){
				evasion *= 0.67f;
			}
		}
	...
	public float stealthFactor( Char owner, float stealth ){
		...
		if (hasGlyph(Obfuscation.class, owner)){
            ...
            if(glyph!=null && glyph.getClass()==Obfuscation.class){
                stealth += (1 + buffedLvl()/3f) * glyph.procChanceMultiplier(owner);
            }

            if(owner==hero && ring!=null && ring.isEquipped(hero) && ring.glyph!=null && ring.glyph.getClass()==Obfuscation.class){
                stealth += (1 + ring.buffedLvl()/3f) * ring.glyph.procChanceMultiplier(owner);
            }
	...
	public int proc( Char attacker, Char defender, int damage ) {
		...
		if (defender.buff(MagicImmune.class) == null) {
			...
			if (defender instanceof Hero && isEquipped((Hero) defender)
                ...
                if (ringGlyph != null){
                    damage = ringGlyph.proc( ring.getArmor(), attacker, defender, damage );
                }
				int blocking = ((Hero) defender).subClass.is(HeroSubClass.PALADIN) ? 3 : 1;
				...
				if (defender.alignment == Dungeon.hero.alignment
					...
					int blocking = Dungeon.hero.subClass.is(HeroSubClass.PALADIN) ? 3 : 1;
					damage -= Math.round(blocking * Glyph.genericProcChanceMultiplier(defender));
				}
	...
	public int STRReq(int lvl){
		...
		if(tier>1 && Dungeon.isChallenged(Challenges.NO_ARMOR) && Dungeon.isChallenged(Challenges.HARSH_ENVIRONMENT)){
			req += 1;
		}
		return req;
	}
	...
	public Armor inscribe( Glyph glyph ) {
		...
		if (glyph != null && isIdentified() && hero != null
	...
	public boolean hasGlyph(Class<?extends Glyph> type, Char owner) {
		if (owner.buff(MagicImmune.class) != null) {
			...
			if(glyph.getClass() == type){
				return true;
			}else if(owner==hero && hero.belongings!=null){
				RingOfKing ring = hero.belongings.getItem(RingOfKing.class);
				if (ring != null && ring.isEquipped(hero) && ring.glyph != null && !ring.cursed && ring.glyph.getClass() == type){
					return true;
				}
			}
			return false;
            ...
            if(owner==hero && hero.belongings!=null){
                RingOfKing ring = hero.belongings.getItem(RingOfKing.class);
                if (ring != null && ring.isEquipped(hero) && ring.glyph != null && !ring.cursed && ring.glyph.getClass() == type){
                    return true;
                }
	...
	public static abstract class Glyph implements Bundlable {
		...
		public static float genericProcChanceMultiplier( Char defender ){
            ...
            if(defender==hero && hero.subClass.is(HeroSubClass.COMBATMASTER) && hero.hasTalent(Talent.INELEMENT)){
                FightStance fightStance = defender.buff(FightStance.class);
                if(fightStance!=null && fightStance.duration>=25-5*hero.pointsInTalent(Talent.INELEMENT)){
                    multi += 0.33f*hero.pointsInTalent(Talent.INELEMENT);
                }
            }
		...
		public String desc() {
            ...
            if(hero!=null && hero.subClass.is(HeroSubClass.COMBATMASTER) && !curse()){
                desc += "\n\n"+Messages.get(this, "combat");
            }
            return desc;
		}
