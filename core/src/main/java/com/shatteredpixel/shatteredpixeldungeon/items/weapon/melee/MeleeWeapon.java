...
public class MeleeWeapon extends Weapon {
	...
	public void onThrow( int cell) {
		...
		if(hero.pointsInTalent(Talent.STRONG_THROW)>0 && enemy!=null){
			enemy.damage(this.tier*hero.pointsInTalent(Talent.STRONG_THROW)*2,hero);
		}
		if(hero.pointsInTalent(Talent.WEIRD_THROW)>0 && enemy!=null){
	...
	public static class Charger extends Buff implements ActionIndicator.Action {
		...
		public boolean act() {
			if (charges < chargeCap()){
				int cnt=((Hero)target).pointsInTalent(Talent.ENERGY_CONVERSION);
				...
				if (Regeneration.regenOn()){
					...
					chargeToGain *= Math.max(1,speed*0.5*cnt);
		...
		public int chargeCap(){
			...
			if (hero.subClass == HeroSubClass.CHAMPION){
				...
				return hero.pointsInTalent(Talent.MARTIAL_TRAIN);
			}else{
				return 0;
			}
