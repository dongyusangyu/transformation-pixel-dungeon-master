		...
		hero.sprite.parent.add(new Chains(hero.sprite.center(),
			...
			public void call() {
				Actor.add(new Pushing(enemy, enemy.pos, pulledPos, new Callback() {
					public void call() {
						enemy.pos = pulledPos;
						if(hero.pointsInTalent(Talent.STRENGTHEN_CHAIN)>=1){
							Buff.affect(enemy,Cripple.class,5);
						}


						charge -= chargeUse;
						if(hero.pointsInTalent(Talent.STRENGTHEN_CHAIN)==2 && chargeUse>1){
							charge+=1;
						}
						...
						if(hero.pointsInTalent(Talent.STRENGTHEN_CHAIN)<2){
	...
	public class chainsRecharge extends ArtifactBuff{
		...
		public boolean act() {
			...
			if (charge < chargeTarget
				...
				chargeGain *= RingOfEnergy.artifactChargeMultiplier(target)*Math.max(1,(hero.speed())*0.5*hero.pointsInTalent(Talent.ENERGY_CONVERSION));
