...
public class Honeypot extends Item {
	...
	public Item shatter( Char owner, int pos ) {
		...
		if (newPos != -1) {
			if(hero.hasTalent(Talent.HONEY_FISH)){
				if(hero.pointsInTalent(Talent.HONEY_FISH)==2){
					new AquaBrew().shatter(newPos);
				}
				Dungeon.level.setCellToWater(true, newPos);
				Mob piranha = Piranha.random();
				Buff.affect(piranha,Amok.class,50);
				piranha.pos=newPos;
