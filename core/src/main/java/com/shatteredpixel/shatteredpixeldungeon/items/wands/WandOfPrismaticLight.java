...
public class WandOfPrismaticLight extends DamageWand {
	...
	public void onZap(Ballistica beam) {
		...
		if (Dungeon.level.viewDistance < 6 && curUser.buffs(Light.class).isEmpty()){
	...
	private void affectTarget(Char ch){
		...
		if(hero.hasTalent(Talent.LIGHT_SHOCK) && ch.buff(Blindness.class)!=null){
			if(ch.buff(Cripple.class)==null) Buff.affect(ch, Cripple.class,buffedLvl()/2);
			if(hero.pointsInTalent(Talent.LIGHT_SHOCK)==2){
				dmg = (int)(dmg*1.3f);
			}
		}
		...
		if (ch.properties().contains(Char.Property.DEMONIC) || ch.properties().contains(Char.Property.UNDEAD)){
			...
			ch.damage(Math.round(dmg*1.333f), this, DamageTag.MAGICAL);
		} else {
