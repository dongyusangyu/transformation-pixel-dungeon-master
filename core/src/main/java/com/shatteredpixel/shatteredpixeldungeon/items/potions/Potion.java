...
public class Potion extends Item {
		...
		if (action.equals( AC_CHOOSE )){
			...
			if (isKnown() && mustThrowPots.contains(getClass())) {
					...
					if(hero.pointsNegative(Talent.CHILL_WATER)*3> Random.Int(20)
							&& hero.pointsNegative(Talent.CHILL_WATER)==2){
						Buff.affect(hero, Chill.class,3);
						}

					if(hero.hasTalent(Talent.WATER_ISFOOD)){
						Buff.affect(hero, Hunger.class).affectHunger(30);
	...
	protected void drink( Hero hero ) {
		...
		if (!anonymous) {
			...
			if(hero.hasTalent(Talent.BOOZY)){
				Buff.affect(hero, Healing.class).setHeal(5*hero.pointsInTalent(Talent.BOOZY),0.2f,1);
				Barkskin.conditionallyAppend(hero, 5*hero.pointsInTalent(Talent.BOOZY), 1 );
			}
