...
public class ChaliceOfBlood extends Artifact {
	...
	private int minPrickDmg(){
		int damage = (int)Math.ceil(3 + 2.5f*(level()*level()));
		if(hero.pointsNegative(Talent.WEAKEN_CHALICE)>0){
			damage+=(int)(0.5f*hero.pointsNegative(Talent.WEAKEN_CHALICE)*damage);
		}
		return damage;
	}

	private int maxPrickDmg(){
		int damage = (int)Math.floor(7 + 3.5f*(level()*level()));
		if(hero.pointsNegative(Talent.WEAKEN_CHALICE)>0){
			damage+=(int)(0.5f*hero.pointsNegative(Talent.WEAKEN_CHALICE)*damage);
		}
		return damage;
	...
	private void prick(Hero hero){
		...
		if (hero.buff(MagicImmune.class) != null && hero.buff(HolyWard.HolyArmBuff.class) != null){
			damage -= hero.subClass.is(HeroSubClass.PALADIN) ? 3 : 1;
		}
		...
		hero.damage(damage, this, DamageTag.PHYSICAL);
