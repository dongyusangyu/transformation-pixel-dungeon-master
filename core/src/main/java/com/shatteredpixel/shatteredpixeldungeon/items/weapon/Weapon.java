...
abstract public class Weapon extends KindOfWeapon {
	...
	public int proc( Char attacker, Char defender, int damage ) {
		...
		if (attacker.buff(MagicImmune.class) == null) {
            ...
            Enchantment ringEnchant = null;
            RingOfKing ring = hero.belongings.getItem(RingOfKing.class);
            if (ring != null && ring.enchantment != null && !ring.cursed && ring.isEquipped(hero) && (attacker == Dungeon.hero || attacker instanceof MirrorImage || attacker instanceof ShadowClone.ShadowAlly)){
			...
			if (attacker instanceof Hero && isEquipped((Hero) attacker)
                ...
                if (hero.subClass.is(HeroSubClass.RUNEMAGE) && attacker==hero && defender!=hero){
					RuneMark runeMark = defender.buff(RuneMark.class);
                    if(runeMark==null ){
                        runeMark = Buff.affect(defender, RuneMark.class);
                    }

                    if(enchantment!=null){
                        runeMark.addEnchantment(enchantment);
                    }
                    if(ringEnchant!=null){
                        runeMark.addEnchantment(ringEnchant);
                    }
                    if(trinityEnchant!=null){
                        runeMark.addEnchantment(trinityEnchant);
                    }
                    if(runeMark.enchantments.isEmpty()){
                        runeMark.detach();
                    }else{
                        defender.sprite.showStatusWithIcon(CharSprite.ORANGE, Messages.get(RuneMark.class,"name"),  FloatingText.NO_ICON);
                        if(hero.pointsInTalent(Talent.DARKMARK)>2){
                            Buff.affect(defender, Weakness.class,3);
                        }
                        if(hero.pointsInTalent(Talent.DARKMARK)>1){
                            Buff.affect(defender, Vulnerable.class,3);
                        }
                        if(hero.pointsInTalent(Talent.DARKMARK)>0){
                            Buff.affect(defender, Hex.class,3);
                        }
                    }
				}
                if (enchantment!=null && attacker instanceof Hero &&
                        (((Hero) attacker).subClass.is(HeroSubClass.PALADIN) || hasCurseEnchant())){
                ...
                if (defender.isAlive() && !becameAlly && ringEnchant != null){
                    damage = ringEnchant.proc(ring.getWeapon(), attacker, defender, damage);
                }
	...
	protected static int STRReq(int tier, int lvl){
		lvl = Math.max(0, lvl);
		int str=(8 + tier * 2) - (int)(Math.sqrt(8 * lvl + 1) - 1)/2;
	...
	public boolean hasEnchant(Class<?extends Enchantment> type, Char owner) {
		if(owner == null ){
			...
			return true;
		} else if(owner instanceof Hero && ((Hero) owner).belongings!=null && ((Hero) owner).belongings.getItem(RingOfKing.class) != null &&
                ((Hero) owner).belongings.getItem(RingOfKing.class).enchantment != null && ((Hero) owner).belongings.getItem(RingOfKing.class).enchantment.getClass()==type
                && ((Hero) owner).belongings.getItem(RingOfKing.class).isEquipped(hero)){
            return true;
	...
	public static abstract class Enchantment implements Bundlable {
		...
		public static final Class<?>[] uncommon = new Class<?>[]{
                ...
                CorrosionEnchanted.class};

		public static final Class<?>[] rare = new Class<?>[]{
				Corrupting.class, Grim.class, Vampiric.class, Sweeping.class};
		...
		public static float genericProcChanceMultiplier( Char attacker ){
            ...
            if(attacker.buff(Talent.EnchantMeal.class) != null){
                multi *= 1.33f;
                attacker.buff(Talent.EnchantMeal.class).left--;
                if(attacker.buff(Talent.EnchantMeal.class).left <= 0){
                    attacker.buff(Talent.EnchantMeal.class).detach();
                }
            }
            if(attacker.buff(RuneMark.RuneSurge.class) != null && hero.pointsInTalent(Talent.RUNE_SURGE)>0){
                multi *= 1.0f+ 1.0f/3*hero.pointsInTalent(Talent.RUNE_SURGE);
            }
		...
		public String desc() {
            ...
            if(hero!=null && hero.subClass.is(HeroSubClass.RUNEMAGE) && !curse()){
                desc += "\n\n"+Messages.get(this, "runemark");
            }
			return desc;
		}
