...
public class DriedRose extends Artifact {
	...
	public class roseRecharge extends ArtifactBuff {
		...
		public boolean act() {
			...
			if (ghost != null && !cursed && target.buff(MagicImmune.class) == null){
				...
				if (ghost.HP < ghost.HT && Regeneration.regenOn()) {
					...
					while (partialCharge > 1) {
						int h=ghost.heal((int)partialCharge);
			...
			if (charge < chargeCap
				...
				partialCharge += (float)(1/5f * RingOfEnergy.artifactChargeMultiplier(target));
				while (partialCharge > 1){
					charge++;
					partialCharge--;
					if (charge == chargeCap){
	...
	public static class GhostHero extends DirectableAlly {
		...
		public void wander(Char ch) {
			yell(Messages.get(this, "directed_wander_"+ Random.IntRange(1, 4)));
			super.wander(ch);
		}
		...
		public void sayAppeared(){
			if (Dungeon.hero.buff(AscensionChallenge.class) != null){
				...
				if(Dungeon.depth % 5 != 1 && Random.Int(5)==1 && hero.hasTalent(Talent.MAGIC_GIRL) && !hero.buffs(HeroDisguise.class).isEmpty()){
					switch (Random.Int(3)){
						case 0:
							yell(Messages.get(this, "magic_girl1" ));
							break;
						case 1:
							yell(Messages.get(this, "magic_girl2" ));
							break;
						case 3:
							yell(Messages.get(this, "magic_girl3" ));
							break;
					}
				}
				...
				case 0:
					for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
						if(mob instanceof Goo){
							...
							break;
						}else if(mob instanceof WarriorBoss){
							yell( Messages.get( this, "seen_warrior_" + Random.IntRange(1, 3) ));
							break;
						}
					}
				...
				case 1:
					for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
						if(mob instanceof Tengu){
							...
							break;
						}else if(mob instanceof RogueBoss){
							yell( Messages.get( this, "seen_rogue_" + Random.IntRange(1, 3) ));
							break;
						}
					}
