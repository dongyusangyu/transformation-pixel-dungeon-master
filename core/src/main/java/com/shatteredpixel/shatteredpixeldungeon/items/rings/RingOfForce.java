	...
	public static int damageRoll( Hero hero ){
		if (hero.buff(Force.class) != null
			...
			if (hero.buff(BrawlersStance.class) != null
				...
				boost = Math.round(3+tier+(level*((4+2*tier)/8f)));
			}
			int dmg = Hero.heroDamageIntRange(min(level, tier) + boost, max(level, tier) + boost);

			//GLog.i("(" + (min(level, tier)+boost) + "-" + (max(level, tier)+boost) + ")");
			SpearShield spsh = Dungeon.hero.belongings.getItem(SpearShield.class);

			if (spsh != null) {
				//GLog.i("->("+spsh.changeDmgMin(min(level, tier) + boost, max(level, tier) + boost)+"-"+spsh.changeDmgMax(min(level, tier) + boost, max(level, tier) + boost)+")");
				dmg = Hero.heroDamageIntRange(
						spsh.changeDmgMin(
								min(level, tier) + boost,
								max(level, tier) + boost),
						spsh.changeDmgMax(
								min(level, tier) + boost,
								max(level, tier) + boost));
			}
