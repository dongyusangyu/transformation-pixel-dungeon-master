...
import com.shatteredpixel.shatteredpixeldungeon.items.scrolls.exotic.ScrollOfUpTier;
...
public class RingOfWealth extends Ring {
	...
	public static ArrayList<Item> tryForBonusDrop(Char target, int tries ){
		...
		while ( triesToDrop.count() <= 0 ){
			if (dropsToEquip.count() <= 0){
                ...
                if(target.buffs(Wealth.class).size()>1 && hero.hasTalent(Talent.RING_BOND) && hero.heroClass != HeroClass.PRINCESS){
                    equipBonus += hero.pointsInTalent(Talent.RING_BOND);
                }
				Item i;
				do {
	...
	public static Item genHighValueConsumable(){
		switch (Random.Int(4)){
			...
			case 3:
				return Random.Float() < ExoticCrystals.consumableExoticChance() ? new ScrollOfUpTier() : new ScrollOfTransmutation();
		}
	}
