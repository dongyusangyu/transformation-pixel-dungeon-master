...
public class ScrollOfRemoveCurse extends InventoryScroll {
	...
	public void doRead() {
		...
		if (spirit != null){
			...
			if((hero.pointsNegative(Talent.ETERNAL_CURSE)==2 && quantity>=2) || hero.pointsNegative(Talent.ETERNAL_CURSE)!=2){
				super.doRead();
				if(hero.pointsNegative(Talent.ETERNAL_CURSE)==2){
					curItem.detach(curUser.belongings.backpack);
				}

			}else{
				GLog.w("负面天赋过强，无法祛邪。");
				//GLog.i( Messages.get(Talent.ETERNAL_CURSE, "unremove") );
			}
		}
	}
		...
		for (Item item : items) {
			...
			if (item instanceof Weapon){
				...
				if (w.hasCurseEnchant()){
					if(Dungeon.hero.pointsNegative(Talent.ETERNAL_CURSE)!=1){
						w.enchant(null);
					}
					procced = true;
				}
            ...
            if(item instanceof Ring && hero!=null &&  hero.belongings.artifact()!=null && hero.belongings.artifact()==item){
                if (item.cursed) {
                    procced = true;
                    item.cursed = false;
                }
