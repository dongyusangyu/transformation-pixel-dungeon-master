...
import static com.shatteredpixel.shatteredpixeldungeon.actors.hero.Talent.MORE_RING;
...
public abstract class KindofMisc extends EquipableItem {
	...
	public boolean doEquip(final Hero hero) {
		...
		if ( this instanceof Artifact
            ...
            if(this instanceof RingOfKing && hero.hasTalent(Talent.MORE_RING) && hero.belongings.artifact == null){
                equipFull = false;
            }else if(hero.pointsInTalent(Talent.MORE_RING)>1 && hero.belongings.artifact == null){
                equipFull = false;
            }
        }
		...
		if (equipFull) {
			...
			if (this instanceof Ring && hero.belongings.misc!=null){
				enabled[0] = false; //disable artifact

                if(hero.hasTalent(Talent.MORE_RING)){
                    if(this instanceof RingOfKing){
                        enabled[0] = true;
                    }else if(hero.pointsInTalent(Talent.MORE_RING)>1){
                        enabled[0] = true;
                    }else{
                        enabled[0] = false;
                    }
						...
						protected void onSelect(int index) {
							...
							if (equipped.doUnequip(hero, true, false)) {
								...
								if (index == 0 && KindofMisc.this instanceof Ring){
									//hero.belongings.artifact = (KindofMisc)hero.belongings.misc;
									//hero.belongings.misc = null;
			...
			if (this instanceof Artifact){
                ...
                if(this instanceof RingOfKing && hero.hasTalent(Talent.MORE_RING) && hero.belongings.artifact == null){
                    hero.belongings.artifact = (Ring) this;
                }else if(hero.pointsInTalent(Talent.MORE_RING)>1 && hero.belongings.artifact == null){
                    hero.belongings.artifact = (Ring) this;
                }else if (hero.belongings.ring == null)   hero.belongings.ring = (Ring) this;
