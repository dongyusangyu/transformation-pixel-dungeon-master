...
public class Ring extends KindofMisc {
	...
	public static int getBonus(Char target, Class<?extends RingBuff> type){
		...
		for (RingBuff buff : target.buffs(type)) {
            ...
            if(buff.buffedLvl()>soloBonus){
                soloBonus = buff.buffedLvl();
            }
		}
	...
	public int soloBonus(){
		if (cursed){
			if(hero!=null && hero.pointsNegative(Talent.MAL_CURSE)>0){
				return Math.min( 0, Ring.this.level()-2-hero.pointsNegative(Talent.MAL_CURSE) );
			}
