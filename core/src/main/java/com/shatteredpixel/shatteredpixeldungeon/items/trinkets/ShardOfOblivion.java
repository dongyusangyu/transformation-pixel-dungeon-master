	...
	public static WndBag.ItemSelector identifySelector = new WndBag.ItemSelector() {
		...
		public void onSelect(Item item) {
			...
			if (item instanceof Weapon){
                ...
                if (curUser.pointsInTalent(Talent.ADVENTURERS_INTUITION) == 2){
                    ready = true;
                }
            ...
            if(curUser.pointsInTalent(Talent.WIDE_KNOWLEDGE)>=2 && !item.isIdentified()){
                ready = true;
            }
            if (curUser.pointsInTalent(Talent.MANA_SENSE)>1){
