...
public class ScrollOfTransmutation extends InventoryScroll {
	...
	protected boolean usableOnItem(Item item) {
		...
		//all regular or exotic scrolls, except itself (unless un-ided, in which case it was already consumed)
			...
			return (item != this || item.quantity() > 1 || identifiedByUse) && item.getClass() != ScrollOfMetamorphosis.class;
			...
			if (result != item) {
				...
				if (item.isEquipped(hero)) {
					...
					if (item instanceof Artifact && result instanceof Ring){
                        ...
                        if(hero.pointsInTalent(Talent.MORE_CHANCE)<2 && hero.belongings.artifact()==item && result instanceof Ring){
                            unequipped = false;
                        }
						((EquipableItem) item).doUnequip(hero, false);
