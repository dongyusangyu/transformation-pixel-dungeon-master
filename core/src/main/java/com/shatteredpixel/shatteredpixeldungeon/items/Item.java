...
public class Item implements Bundlable {
	...
	public ArrayList<String> actions( Hero hero ) {
		...
		if (Talent.canRavensEyeInspect(hero, this)){
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals( AC_DROP )) {
			...
			if (hero.belongings.backpack.contains(this) || isEquipped(hero)) {
				Talent.doRavensEyeInspect(hero, this);
			}
	...
	protected void onThrow( int cell ) {
		...
		if(!(this instanceof Weapon) && hero.hasTalent(Talent.FEINT) && curUser==hero && hero.buff(Talent.FeintCooldown.class)==null){
			...
			Buff.affect(hero,Talent.FeintCooldown.class,25-hero.pointsInTalent(Talent.FEINT)*5);
		}
	}
	...
	public int buffedLvl(){
		int minlvl=0;
		if(hero!=null && hero.pointsNegative(Talent.MAL_CURSE)>0 && this.cursed){
			minlvl-=hero.pointsNegative(Talent.MAL_CURSE);
		}
		...
		if(hero != null && hero.buff( MostDegrade.class) != null
			...
			&& (isEquipped( hero ) || hero.belongings.contains( this ))) {
			return Degrade.reduceLevel(level())+minlvl;
		}else if(this instanceof EquipableItem || this instanceof Wand){
			return level()+minlvl;
		}else{
			return level();
		}
