...
public class WandOfLivingEarth extends DamageWand {
	...
	public void onHit(MagesStaff staff, Char attacker, Char defender, int damage) {
		...
		if (guardian != null){
			guardian.sprite.centerEmitter().burst(MagicMissile.EarthParticle.ATTRACT, 8 + buffedLvl() / 2);
			guardian.setInfo(hero, buffedLvl(), armor);
		} else {
	...
	public static class EarthGuardian extends NPC {
		...
		private class Wandering extends Mob.Wandering{
			...
			public boolean act(boolean enemyInFOV, boolean justAlerted) {
				if (!enemyInFOV){
					Buff.affect(hero, RockArmor.class).addArmor(wandLevel, HP);
					if (buff(PowerOfMany.PowerBuff.class) != null){
						Buff.affect(hero, RockArmor.class).powerOfManyTurns = buff(PowerOfMany.PowerBuff.class).cooldown()+1;
					}
					hero.sprite.centerEmitter().burst(MagicMissile.EarthParticle.ATTRACT, 8 + wandLevel/2);
					destroy();
					sprite.die();
					return true;
				} else {
