...
abstract public class ClassArmor extends Armor {
	...
	public static ClassArmor upgrade (Hero owner, Armor armor ) {
		...
		switch (owner.heroClass) {
			...
			case NINJA:
				...
				break;
			case DM400:
				classArmor = new DM400Armor();
				break;
            case PRINCESS:
                classArmor = new PrincessArmor();
                break;
            case FRIAR:
                classArmor = new FriarArmor();
                break;
		}
	...
	public ArrayList<String> actions( Hero hero ) {
		...
		if(this instanceof SlimeGirlArmor){
		...
		public boolean act() {
			if (Regeneration.regenOn()) {
				...
				if(hero.speed()>1 && cnt>=1){
					chargeGain *=Math.max(1,(hero.speed())*0.5*cnt);
				}
				charge += chargeGain;
				updateQuickslot();
				if (charge > 100) {
