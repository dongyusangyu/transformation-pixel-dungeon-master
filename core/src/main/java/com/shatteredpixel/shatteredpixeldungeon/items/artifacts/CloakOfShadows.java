...
public class CloakOfShadows extends Artifact {
		...
		public boolean act() {
			if (charge < chargeCap && !cursed && target.buff(MagicImmune.class) == null) {
				if (activeBuff == null && Regeneration.regenOn()) {
					...
					if(hero.pointsInTalent(Talent.STRENGTH_CLOAK)==3){
						chargeToGain*=1.2f;
					}
					if (!isEquipped(hero)){
	...
	public class cloakStealth extends ArtifactBuff{
		...
		public boolean act(){
			...
			if (turnsToCost <= 0){
				...
				if (charge < 0) {
					...
					turnsToCost = 4;
					if(hero.hasTalent(Talent.STRENGTH_CLOAK)){
						turnsToCost+=0.5f+0.5f*hero.pointsInTalent(Talent.STRENGTH_CLOAK);
					}
				}
				updateQuickslot();
			}
		...
		public void detach() {
			...
			if(target==hero && hero.hasTalent(Talent.PHANTOM_STEP)){
                if(hero.buff(Haste.class)!=null) Buff.affect(hero, Haste.class,Math.min(1+2*hero.pointsInTalent(Talent.PHANTOM_STEP),(1+2*hero.pointsInTalent(Talent.PHANTOM_STEP))*2-hero.buff(Haste.class).getturns()));
				else Buff.affect(hero, Haste.class,1+2*hero.pointsInTalent(Talent.PHANTOM_STEP));
				Buff.affect(hero,Talent.PhantomStep.class,hero.pointsInTalent(Talent.PHANTOM_STEP));
			}
			updateQuickslot();
			super.detach();
		}
