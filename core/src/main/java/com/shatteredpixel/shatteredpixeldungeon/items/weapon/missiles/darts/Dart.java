...
public class Dart extends MissileWeapon {
	...
	public int min(int lvl) {
		if (bow != null){
            ...
            if (!(this instanceof TippedDart) && Dungeon.hero.buff(Blowpipe.WeakChargedShot.class) != null) {
                //ability increases base dmg by 50%, scaling by 50%
                return  6 +                       //16 base
                        2 * pipe.buffedLvl() + lvl; //+4 per bow level, +2 per level
            }else {
                return  4 +                     //4 base
                        pipe.buffedLvl() + lvl;  //+1 per level or bow level
            }
        } else {
	...
	public int max(int lvl) {
		if (bow != null){
            ...
            if (!(this instanceof TippedDart) && Dungeon.hero.buff(Blowpipe.WeakChargedShot.class) != null) {
                //ability increases base dmg by 50%, scaling by 50%
                return 12 +                       //16 base
                        3 * pipe.buffedLvl() + 2 * lvl; //+4 per bow level, +2 per level
            }
            else{
                return 10 +                       //12 base
                        2 * pipe.buffedLvl() + 2 * lvl; //+3 per bow level, +2 per level
            }
        } else {
	...
	protected static Blowpipe pipe;
	...
	private void updateBlowpipe(){
		if (Dungeon.hero == null) {
			pipe = null;
		} else if (Dungeon.hero.belongings.weapon() instanceof Blowpipe){
			pipe = (Blowpipe) Dungeon.hero.belongings.weapon();
		} else if (Dungeon.hero.belongings.secondWep() instanceof Blowpipe) {
			//player can instant swap anyway, so this is just QoL
			pipe = (Blowpipe) Dungeon.hero.belongings.secondWep();
		} else {
			pipe = null;
		}
	}
	...
	public boolean hasEnchant(Class<? extends Enchantment> type, Char owner) {
		if (bow != null && bow.hasEnchant(type, owner)){
			return true;
		} else if (pipe != null && pipe.hasEnchant(type, owner)){
			return true;
		} else {
	...
	public float accuracyFactor(Char owner, Char target) {
		...
		} else if (pipe != null && owner.buff(Blowpipe.WeakChargedShot.class) != null){
			return Char.INFINITE_ACCURACY;
		} else {
			float accuracy = super.accuracyFactor(owner, target);
			if (owner instanceof Hero && target != null && target.alignment != owner.alignment){
				accuracy *= Talent.bullseyeAccuracyFactor((Hero) owner);
			}
			return accuracy;
		}
	}
	...
	public int proc(Char attacker, Char defender, int damage) {
		...
		if (pipe != null && !processingChargedShot) {
			damage = pipe.proc(attacker, defender, damage);
		}
	...
	protected void processChargedShot( Char target, int dmg ){
		...
		if (chargedShotPos != -1 &&(( bow != null && Dungeon.hero.buff(Crossbow.ChargedShot.class) != null )||( pipe != null && Dungeon.hero.buff(Blowpipe.WeakChargedShot.class) != null))) {
	...
	protected void decrementDurability() {
		...
		if (Dungeon.hero.buff(Blowpipe.WeakChargedShot.class) != null) {
			Dungeon.hero.buff(Blowpipe.WeakChargedShot.class).detach();
		}
	}
	...
	public void throwSound() {
		...
		if (bow != null) {
			...
			Sample.INSTANCE.play(Assets.Sounds.BLOWPIPE, 1, Random.Float(0.87f, 1.15f));
		} else {
			super.throwSound();
		}
	}
	...
	public String info() {
		...
		if (bow != null && !bow.isIdentified()){
			...
			bow = realBow;
			return info;
		}else if (pipe != null && !pipe.isIdentified()){
			Blowpipe realPipe = pipe;
			//create a temporary bow for IDing purposes
			pipe = new Blowpipe();
			String info = super.info();
			pipe = realPipe;
