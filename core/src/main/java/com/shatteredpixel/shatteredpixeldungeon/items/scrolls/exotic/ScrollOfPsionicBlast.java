...
public class ScrollOfPsionicBlast extends ExoticScroll {
	...
	public void doRead() {
		...
		for (Mob mob : targets){
			...
			mob.damage(Math.round(mob.HT/2f + mob.HP/2f), this, DamageTag.MAGICAL);
			if (mob.isAlive()) {
				Buff.prolong(mob, Blindness.class, Blindness.DURATION);
			}
		}
		
		curUser.damage(Math.max(0, Math.round(curUser.HT*(0.5f * (float)Math.pow(0.9, targets.size())))), this, DamageTag.MAGICAL);
		if (curUser.isAlive()) {
