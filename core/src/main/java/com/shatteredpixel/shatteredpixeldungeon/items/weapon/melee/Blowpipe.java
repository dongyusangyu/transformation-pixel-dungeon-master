...
public class Blowpipe extends MeleeWeapon {
    ...
    public boolean doUnequip(Hero hero, boolean collect, boolean single) {
        if (super.doUnequip(hero, collect, single)){
            if (hero.buff(WeakChargedShot.class) != null &&
                ...
                hero.buff(WeakChargedShot.class).detach();
            }
            return true;
        } else {
            return false;
    ...
    public int dartMax(int lvl){
        return  10 +    //12 base, up from dart base of 2
                2*lvl;  //+3 per crossbow level
    }

    @Override
    public float accuracyFactor(Char owner, Char target) {
        if (owner.buff(WeakChargedShot.class) != null){
            Actor.add(new Actor() {
    ...
    public int proc(Char attacker, Char defender, int damage) {
        ...
        if (attacker.buff(WeakChargedShot.class) != null && !(curItem instanceof Dart)){
            ...
            WandOfBlastWave.throwChar(defender,
                    ...
                    true,
                    this);
            attacker.buff(WeakChargedShot.class).detach();
        }
        return dmg;
    }
    ...
    protected void duelistAbility(Hero hero, Integer target) {
        ...
        Buff.affect(hero, WeakChargedShot.class);
    ...
    public String abilityInfo() {
        if (levelKnown){
            return Messages.get(this, "ability_desc", 2+buffedLvl(), 2+buffedLvl());
        } else {
            return Messages.get(this, "typical_ability_desc", 2, 2);
        }
    }
