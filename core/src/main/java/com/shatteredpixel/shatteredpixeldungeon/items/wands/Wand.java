...
public abstract class Wand extends Item {
	...
	public void wandUsed() {
		...
		if(hero.pointsNegative(Talent.OUTCONTROL_MAGIC)>Random.Int(5)){
			new Bomb.ConjuredBomb().explode(curUser.pos);
		}
		...
		if(hero.pointsInTalent(Talent.MAGIC_RECYCLING)>0 && hero.pointsInTalent(Talent.MAGIC_RECYCLING)*5+5>Random.Int(100)){
			...
			int c = chargesPerCast();
			if(this instanceof WandOfFireblast && hero != null && hero.pointsInTalent(Talent.DEVIL_FLAME)>1 && c>1){
				c--;
	...
	public class Charger extends Buff {
		...
		private void recharge(){
			...
			if (Regeneration.regenOn())
				partialCharge += (gainCharge) * RingOfEnergy.wandChargeMultiplier(target)*Math.max(1,(hero.speed())*0.5*cnt);
