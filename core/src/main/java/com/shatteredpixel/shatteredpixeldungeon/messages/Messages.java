...
public class Messages {
	...
	private static ArrayList<I18NBundle> directBundles;
	private static ArrayList<I18NBundle> chineseBundles;
	...
	public static void setup( Languages lang ){
		...
		directBundles = new ArrayList<>();
		chineseBundles = new ArrayList<>();
	...
	public static String get(Class c, String k, Object...args){
		...
		if (value != null){
			...
			if (c != null && c.getSuperclass() != null){
				value = get(c.getSuperclass(), k, args);
                return value;
                /*
				if (!value.equals(c.getSuperclass().getName().replace("com.shatteredpixel.shatteredpixeldungeon.", "").toLowerCase(Locale.ENGLISH) + "." + k.toLowerCase(Locale.ENGLISH))){
    ...
    public static  boolean canget(Class c, String k, Object...args){
        String key;
        if (c != null){
            key = c.getName().replace("com.shatteredpixel.shatteredpixeldungeon.", "");
            key += "." + k;
	...
	private static String getFromBundle(String key){
		...
		for (int i = 0; i < bundles.size(); i++){
			result = getIfFound(directBundles.get(i), key);
			if (result != null){
				return result;
			}

			if (lang != Languages.CHI_SMPL){
				result = getIfFound(chineseBundles.get(i), key);
				if (result != null){
					return result;
				}
			}

			result = getIfFound(bundles.get(i), key);
			if (result != null){
				return result;
			}
		}
	...
	private static String getIfFound(I18NBundle bundle, String key){
