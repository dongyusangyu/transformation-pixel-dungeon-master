...
public class WndTradeItem extends WndInfoItem {
	...
	private void buy( Heap heap ) {
		...
		if(hero.hasTalent(Talent.THRID_HAND)){
			if(hero.pointsInTalent(Talent.THRID_HAND)<=Random.Int(10)){
				Dungeon.gold -= price;
				Catalog.countUses(Gold.class, price);
			}else{
				//GLog.p("你趁老板不注意，拾走了这件商品");
				GLog.p(Messages.get(WndTradeItem.class, "hand_steal"));
			}
