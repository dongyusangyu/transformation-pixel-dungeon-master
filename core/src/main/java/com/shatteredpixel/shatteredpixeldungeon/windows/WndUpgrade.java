...
public class WndUpgrade extends Window {
	...
	public WndUpgrade( Item upgrader, Item toUpgrade, boolean force){
		...
		if (toUpgrade instanceof Crossbow){
			bottom = fillFields(Messages.get(this, "dart_damage"),
					...
					bottom);
		}else if (toUpgrade instanceof Blowpipe){
			bottom = fillFields(Messages.get(this, "dart_damage"),
					((Blowpipe) toUpgrade).dartMin(levelFrom) + "-" + ((Blowpipe) toUpgrade).dartMax(levelFrom),
					((Blowpipe) toUpgrade).dartMin(levelTo) + "-" + ((Blowpipe) toUpgrade).dartMax(levelTo),
					bottom);
		}
		...
		btnUpgrade.setRect(0, bottom+2*GAP, WIDTH/3f, 16);
		add(btnUpgrade);

		btnUpgrades = new RedButton(Messages.get(this, "upgrades")){
			@Override
			protected void onClick() {
				super.onClick();
		...
		btnUpgrades.setRect(btnUpgrade.right()+1, bottom+2*GAP, WIDTH/3f, 16);
		if(!force) add(btnUpgrades);
		...
		btnCancel.setRect(WIDTH-WIDTH/3f, bottom+2*GAP, WIDTH/3f, 16);
		add(btnCancel);
		...
		btnUpgrades.enable(Dungeon.hero.ready);
	...
	public synchronized void update() {
		...
		if (!btnUpgrades.active && Dungeon.hero.ready){
			btnUpgrades.enable(true);
		}
	}
