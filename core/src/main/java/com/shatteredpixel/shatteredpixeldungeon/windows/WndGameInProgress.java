		...
		RedButton exportClipboard = new RedButton( Messages.get(this, "export_clipboard")){
			...
			protected void onClick() {
				...
				if (GamesInProgress.copyToClipboard(slot)) {
					ShatteredPixelDungeon.scene().add(new WndMessage(Messages.get(WndGameInProgress.class, "export_success")));
				} else {
					ShatteredPixelDungeon.scene().add(new WndMessage(Messages.get(WndGameInProgress.class, "export_failed")));
				}
			}
		};
