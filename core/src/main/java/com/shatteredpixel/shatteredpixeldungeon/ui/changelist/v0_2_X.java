...
public class v0_2_X {
    ...
    public static void add_v0_2_2Changes( ArrayList<ChangeInfo> changeInfos ) {
        ...
        changes.addButton( new ChangeButton(HeroSprite.avatar(HeroClass.CLERIC, 6), "内容同步",
                "_-_ 于_fix7_进行_破碎v3.0_的内容同步(感谢_委员会全员_以及_墨意-天志_为内容同步更新进行的大量测试)\n" +
                        "_-_ _牧师实装：_牧师实装并对牧师初始天赋进行调整。牧师天赋除启蒙圣餐外，其余天赋被其他英雄蜕变出来均无效果变更，同时启蒙圣餐被其他英雄蜕变出来的效果也进行了改动，而符文复制新增无法复制蜕变卷轴的改动\n" +
    ...
    public static void add_v0_2_4Changes( ArrayList<ChangeInfo> changeInfos ) {
        ...
        changeInfos.add(changes);
        changes.addButton(new ChangeButton(Icons.get(Icons.TALENT), "天赋更新",
                "_-_ 可蜕变的正面天赋划分为_物攻_，_法伤_，_特效_，_资源_，_法术_，_辅助_，_其他_共7类，在使用蜕变卷轴蜕变天赋时可选择类型，该类型天赋将以3倍概率出现在选项中。具体天赋分类请到对应天赋详情页查看\n"+
    ...
    public static void add_v0_2_5Changes( ArrayList<ChangeInfo> changeInfos ) {
        ...
        changes.addButton(new ChangeButton(Icons.get(Icons.BUFFS),"平衡性调整",
                "_-_ 于_fix1_为天赋_无敌一餐_、_饭里藏金_、_吃出经验_和_生日礼物_设置100次触发上限\n" +
                        ...
                        "_-_ 于_fix6_令所有角色开局鉴定蜕变卷轴，同时加强天赋_即兴投掷_，冷却从50下调到20\n"
        ));
        changes.addButton(new ChangeButton(Icons.get(Icons.PREFS), "改动",
    ...
    public static void add_v0_2_6Changes( ArrayList<ChangeInfo> changeInfos ) {
        ...
        changes.addButton(new ChangeButton(Icons.get(Icons.CATALOG), "内容更新",
                "_-_ 于_fix4_新增_2_个物品\n"+
                        "_-_ _蜕变结晶：_它会允许使用者对任一通用天赋进行定向蜕变。可以通过使用蜕变卷轴合成获取\n"+
    ...
    public static void add_v0_2_7fixChanges( ArrayList<ChangeInfo> changeInfos ) {
        ...
        changes.hardlight(Window.TITLE_COLOR);
        changeInfos.add(changes);
        changes = new ChangeInfo( "v0.2.7fix5", false, null);
        changes.hardlight( Window.TITLE_COLOR );
        changeInfos.add(changes);
        changes.addButton(new ChangeButton(Icons.get(Icons.CATALOG), "内容更新",
                        "_-_ 新增共计_4_个负面天赋：吉列豆蒸，炼金事故，晶化躯壳，恶毒诅咒\n"+
                                "_-_ 追加挑战_天赋弱化_、_最大麦穗_、_负面天赋_的挑战效果，追加新生物_蜕变宝箱怪_\n"+
                        "_-_ 破损短剑追加重铸功能\n"+
                                "沉重诅咒追加奥术效果\n"
        ));
        ...
        changes.addButton(new ChangeButton(Icons.get(Icons.PREFS), "改动",
                "_-_  更新日期：2025年12月19日\n"+
                        "_-_  部分游戏内容视觉调整\n"+
                        "_-_  部分文本描述调整\n"+
                        "_-_  盗贼？战中的手里剑初始为鉴定状态\n"
        ));
        changes.addButton(new ChangeButton(Icons.get(Icons.BUFFS),"平衡性调整",
                "_-_ 削弱天赋_杀戮盛宴_：击杀怪物的恢复和神器充能分别从10%/15%和4/6回合下调至5%/7.5%和2/3回合\n" +
                        "_-_ 调整天赋_水灵恢复_：水灵恢复的总量受玩家等级影响，不再因饥饿导致回复量减少\n"+
                        "_-_ 削弱天赋_动能吸收_：现在至少会受到1点伤害\n"+
                        "_-_ 盗贼的初始一层天赋窃贼直觉改为暗影庇护\n"+
                        "盗贼？的隐遁等效果现在可以被物理以外的伤害类型中断\n"
        ));
        changes.addButton(new ChangeButton(new Image(Assets.Sprites.SPINNER, 144, 0, 16, 16),"Bug修复",
                "_-_ 天赋_奇迹炼金_，_冬泳怪鸽_，_新手福利_，_盈能生命_，_恶魔之焰_，_工程技师_，_烟雾掩护_，_极度深寒_，_神圣感知_，_阴影杀手_，_进食过载_，_杀戮盛宴_，_战意盎然_，_经验固化_生效异常问题\n"+
                        "_-_ 修复排行榜显示、视野残留、复活术、炸弹爆炸、复活术、激光无人机攻速、盗贼？战背景音乐消失、闪光无人机伤害类型、升级丢失特效、信息等问题\n"

        ));
        changes = new ChangeInfo( "v0.2.7fix4", false, null);
        changes.hardlight( Window.TITLE_COLOR );
        changeInfos.add(changes);
        changes.addButton( new ChangeButton(new Image(new RogueBossSprite()), "新增10层Boss：盗贼？",
                "_-_ _盗贼？_和_天狗_根据种子有1/2概率生成\n" +
                        "_-_ _盗贼？_拥有刺客和疾行者的部分特性，以及能够使用护甲技能。专属_升华秘卷_并能获得专属成就\n" +
                        "_-_ 新增_4_个盗贼？天赋：阴影杀手，杀戮盛宴，人潮汹涌，幻影迷步\n"+
                        "_-_ 角色由_DM-216_设计，_Secher-Nbiw_制作\n"

        ));
        changes.addButton(new ChangeButton(Icons.get(Icons.CATALOG), "内容更新",
                "_-_ _升变秘卷：_这张秘卷保留了嬗变卷轴的全部魔力，同时还能够提升武器与护甲的等阶！升变魔法在提升武器与护甲等阶的同时，还会保留原物品上的升级、附魔和刻印。\n"+
                        "_-_ 新增共计_7_个正面天赋\n"+
                        "_-_ _1层：_武装暴动，地牢英雄，嗜酒如命\n"+
                        "_-_ _2层：_经验固化，美梦长眠，光明震慑\n" +
                        "_-_ _3层：_财富石像\n"+
                        "_-_ 新增共计_4_个负面天赋：饿鬼道，无法安息，被诅咒者，高利贷\n"+
                        "_-_ 新增图鉴的查询和天赋获取的统计功能，由_Secher-Nbiw_制作\n"
        ));


        changes.addButton(new ChangeButton(Icons.get(Icons.PREFS), "改动",
                "_-_  更新日期：2025年11月22日\n"+
                        "_-_  部分游戏内容视觉调整\n"+
                        "_-_  部分文本描述调整\n"
        ));
        changes.addButton(new ChangeButton(new Image(Assets.Sprites.SPINNER, 144, 0, 16, 16),"Bug修复",
                "_-_ 天赋_动能转换_，_打草惊蛇_，_定期维护_，_赐福圣餐_，_机身加固_，_烈气护盾_，_生命在于运动_生效异常问题\n"+
                        "_-_ 暗液史莱姆复活异常获得忍术问题\n"
