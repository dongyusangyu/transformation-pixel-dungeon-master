...
public class TalentsPane extends ScrollPane {
	...
	public TalentsPane( TalentButton.Mode mode, ArrayList<LinkedHashMap<Talent, Integer>> talents ) {
		...
		if (showAllOwnedTiers) {
			...
			/*
			if (Dungeon.hero.lvl >= 6) tiersAvailable++;
			if (Dungeon.hero.lvl >= 12 && (Dungeon.hero.subClass != HeroSubClass.NONE || Dungeon.hero.heroClass== HeroClass.FREEMAN)) tiersAvailable++;
			if (Dungeon.hero.lvl >= 20 && Dungeon.hero.armorAbility != null) tiersAvailable++;
			...
			if (tiersAvailable > 2 && (Dungeon.hero.subClass.is(HeroSubClass.NONE) && Dungeon.hero.heroClass!= HeroClass.FREEMAN)){
				tiersAvailable = 2;
