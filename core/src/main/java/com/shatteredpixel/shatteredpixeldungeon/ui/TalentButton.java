...
public class TalentButton extends Button {
	...
	public static void addTalent(Talent talent,int pointsInTalent,int tier){
		int cnt=1;
		for (LinkedHashMap<Talent, Integer> tiers : Dungeon.hero.talents){
			if(cnt==tier){
				LinkedHashMap<Talent, Integer> newTier = new LinkedHashMap<>();
				...
				newTier.put(talent, pointsInTalent);
				Dungeon.hero.talents.set(tier-1, newTier);
				Dungeon.hero.negativeTalents.put(talent,tier-1);
				Statistics.negativetalents[tier-1]=true;
				TalentCatalog.countUse(talent);
