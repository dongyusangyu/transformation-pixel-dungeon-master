...
public class HorizontalScrollPane extends Component {
	...
	public void scrollTo(float x, float y) {
		...
		c.scroll.set(x, y);
		clampScroll();
		thumb.x = this.x + width * c.scroll.x / content.width();
	}
	...
	public synchronized void update() {
		super.update();
		if (keyScroll != 0) {
			scrollTo(content.camera.scroll.x + (keyScroll * 150 * Game.elapsed), content.camera.scroll.y);
		}
	}
	...
	protected void layout() {
		...
		thumb.visible = width < content.width();
		if (thumb.visible) {
			thumb.scale.set(width * width / content.width(), 2);
			thumb.x = x + width * content.camera.scroll.x / content.width();
			thumb.y = bottom() - thumb.height();
		}
	}
	...
	public class PointerController extends ScrollArea {
		...
		protected void onClick(PointerEvent event) {
			...
			HorizontalScrollPane.this.onClick(p.x, p.y);
		}
		...
		private void scroll(PointF current) {
			...
			thumb.x = x + width * c.scroll.x / content.width();
