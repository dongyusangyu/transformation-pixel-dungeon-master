...
public class HeroSelectScene extends PixelScene {
	...
	public void create() {
		...
		if (landscape()){
			...
			for (StyledButton button : heroBtns){
				...
				curX += btnWidth+1;
				count++;
				if (count >= (1+heroBtns.size())/3){
					curX = curX1+btnWidth/2f;
					curY += btnHeight+1;
	...
	private class GameOptions extends Component {
        ...
        public StyledButton SkinButton;
		...
		protected void createChildren() {
			...
			if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
				...
				SkinButton = new StyledButton(Chrome.Type.BLANK, Messages.get(WndSkins.class, "title"), 6){
					@Override
					protected void onClick() {
