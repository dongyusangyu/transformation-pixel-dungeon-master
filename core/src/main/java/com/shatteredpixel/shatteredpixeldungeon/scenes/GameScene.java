...
import com.shatteredpixel.shatteredpixeldungeon.sprites.RatKingHeroSprite;
...
public class GameScene extends PixelScene {
	...
	public void create() {
		...
		terrainFeatures = new TerrainFeaturesTilemap(level.plants, level.traps);
		terrain.add(terrainFeatures);

		levelVisuals = level.addVisuals();
		add(levelVisuals);
		...
		hero = Dungeon.hero.heroClass == HeroClass.RATKING ? new RatKingHeroSprite() : new HeroSprite();
		...
		for( CustomTilemap visual : level.customWalls){
			addCustomWall(visual);
		}

		levelWallVisuals = level.addWallVisuals();
		...
		switch (InterlevelScene.mode) {
			case RESURRECT:
				...
				break;
			case RETURN:
				if (level.pit[Dungeon.hero.pos] && !Dungeon.hero.flying){
		...
		if (InterlevelScene.mode != InterlevelScene.Mode.NONE) {
			...
			if (Dungeon.hero.hasTalent(Talent.ROGUES_FORESIGHT)
					&& level instanceof RegularLevel && Dungeon.branch == 0){
				int reqSecrets = level.feeling == Level.Feeling.SECRETS ? 2 : 1;
            ...
            int cnt = 0;
			for (Mob mob : level.mobs) {
				if (!mob.buffs(ChampionEnemy.class).isEmpty()) cnt++;
			}
            if(Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT) && Dungeon.isChallenged(Challenges.STRONGER_BOSSES)){
            }else if(cnt>0){
                GLog.w(Messages.get(ChampionEnemy.class, "warn", cnt));
            }
	...
	private synchronized void addMobSprite( Mob mob ) {
		...
		sprite.visible = level.heroFOV[mob.pos];
		mobs.add( sprite );
		sprite.link( mob );
		sortMobSprites();
	}
	...
	public static void resetMap() {
		if (scene != null) {
			scene.tiles.map(level.map, level.width() );
			scene.visualGrid.map(level.map, level.width() );
			scene.terrainFeatures.map(level.map, level.width() );
			scene.raisedTerrain.map(level.map, level.width() );
			scene.walls.map(level.map, level.width() );
		}
		updateFog();
	}
	...
	public static void afterObserve() {
		if (scene != null) {
			for (Mob mob : level.mobs.toArray(new Mob[0])) {
				if (mob.sprite != null) {
					if (mob instanceof Mimic && mob.state == mob.PASSIVE && ((Mimic) mob).stealthy() && level.visited[mob.pos]){
						//mimics stay visible in fog of war after being first seen
						mob.sprite.visible = true;
					} else {
						mob.sprite.visible = level.heroFOV[mob.pos];
					}
				}
	...
	private static final CellSelector.Listener defaultCellListener = new CellSelector.Listener() {
		...
		public void onRightClick(Integer cell) {
			...
			if (objects.isEmpty()) {
				...
				title = textLines.remove(0);
				image = TerrainFeaturesTilemap.tile(cell, level.map[cell]);
