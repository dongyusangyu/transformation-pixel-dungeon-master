...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		//*** Transform Pixel Dungeon Credits ***

		CreditsBlock tpx = new CreditsBlock(true, Window.TPX_COLOR,
				"Transform Pixel Dungeon",
				Icons.TPX.get(),
		...
		CreditsBlock program = new CreditsBlock(false, Window.TPX_COLOR,
				"Program:",
				null,
				"东隅桑榆\nSecher-Nbiw",
				null,
				null);
		program.setRect(art.left()+5, art.bottom() + 5, colWidth/3f, 0);
		content.add(program);

		CreditsBlock design = new CreditsBlock(false, Window.TPX_COLOR,
				"Design:",
				null,
				"Everyone",
				null,
				null);
		design.setRect(program.right()+25, program.top(), 30, 0);
		content.add(design);

		addLine(program.bottom() + 4, content);
		...
		if (landscape()){
			shpx.setRect((w - fullWidth)/2f - 6, program.bottom()+12, 120, 0);
		} else {
			shpx.setRect((w - fullWidth)/2f, program.bottom()+12, 120, 0);
		}
		content.add(shpx);
