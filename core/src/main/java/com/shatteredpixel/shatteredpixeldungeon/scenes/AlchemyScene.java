		...
		energyAdd = new IconButton(Icons.get(Icons.PLUS)){
			...
			protected void onClick() {
				if(hero!=null && hero.heroClass== HeroClass.FRIAR){
                    Reason r = hero.buff(Reason.class);
                    if(r!=null && r.reason>5){
                        ShatteredPixelDungeon.scene().addToFront(new WndOptions(new ItemSprite(ItemSpriteSheet.ENERGY),
                                Messages.titleCase(Messages.get(Reason.class, "energize_name")),
                                Messages.get(Reason.class, "energize_desc",r.reason),
                                Messages.get(Reason.class, "energize_item"),
                                Messages.get(Reason.class, "energize_1")){
                            @Override
