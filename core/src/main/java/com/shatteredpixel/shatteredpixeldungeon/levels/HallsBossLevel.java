...
public class HallsBossLevel extends Level {
	...
	public void playLevelMusic() {
		if(Statistics.bossScores[4]>0 && locked){
	...
	public void reseal() {
		...
		ChaosDisciples.SuccBoss boss1 = new ChaosDisciples.SuccBoss();
		Buff.affect(boss1, ChaosDisciples.SummonCooldown.class,10);
		boss1.pos = exit() + width*4-1;

		//push any char that is already here away
		if (Actor.findChar(boss1.pos) != null){
			ArrayList<Integer> candidates = new ArrayList<>();
			for (int i : PathFinder.NEIGHBOURS8){
				if (Actor.findChar(boss1.pos + i) == null){
					candidates.add(boss1.pos + i);
				}
			}
			Char ch = Actor.findChar(boss1.pos);
			if (!candidates.isEmpty()){
				ch.pos = Random.element(candidates);
			} else {
				ch.pos = boss1.pos+2*width;
			}
			Actor.add(new Pushing(ch, boss1.pos, ch.pos));
		}

		GameScene.add( boss1 );
		ChaosDisciples.EyeBoss boss2 = new ChaosDisciples.EyeBoss();
		boss2.pos = exit() + width*4+1;

		//push any char that is already here away
		if (Actor.findChar(boss2.pos) != null){
			ArrayList<Integer> candidates = new ArrayList<>();
			for (int i : PathFinder.NEIGHBOURS8){
				if (Actor.findChar(boss2.pos + i) == null){
					candidates.add(boss2.pos + i);
				}
			}
			Char ch = Actor.findChar(boss2.pos);
			if (!candidates.isEmpty()){
				ch.pos = Random.element(candidates);
			} else {
				ch.pos = boss2.pos+2*width;
			}
			Actor.add(new Pushing(ch, boss2.pos, ch.pos));
		}

		GameScene.add( boss2 );

		ChaosDisciples.ScorpioBoss boss3 = new ChaosDisciples.ScorpioBoss();
		boss3.pos = exit() + width*2+1;

		//push any char that is already here away
		if (Actor.findChar(boss3.pos) != null){
			ArrayList<Integer> candidates = new ArrayList<>();
			for (int i : PathFinder.NEIGHBOURS8){
				if (Actor.findChar(boss3.pos + i) == null){
					candidates.add(boss3.pos + i);
				}
			}
			Char ch = Actor.findChar(boss3.pos);
			if (!candidates.isEmpty()){
				ch.pos = Random.element(candidates);
			} else {
				ch.pos = boss3.pos+2*width;
			}
			Actor.add(new Pushing(ch, boss3.pos, ch.pos));
		}

		GameScene.add( boss3 );

		ChaosDisciples.RipperBoss boss4 = new ChaosDisciples.RipperBoss();
		boss4.pos = exit() + width*2-1;

		//push any char that is already here away
		if (Actor.findChar(boss4.pos) != null){
			ArrayList<Integer> candidates = new ArrayList<>();
			for (int i : PathFinder.NEIGHBOURS8){
				if (Actor.findChar(boss4.pos + i) == null){
					candidates.add(boss4.pos + i);
				}
			}
			Char ch = Actor.findChar(boss4.pos);
			if (!candidates.isEmpty()){
				ch.pos = Random.element(candidates);
			} else {
				ch.pos = boss4.pos+2*width;
			}
			Actor.add(new Pushing(ch, boss4.pos, ch.pos));
		}
