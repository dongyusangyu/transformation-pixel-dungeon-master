...
public class PrisonNewBossLevel extends Level{
    ...
    private void setMapArena(int pos){
        ...
        GameScene.add(rogue);
        rogue.beckon(hero.pos);
        if (!BossHealthBar.isAssigned()){
            BossHealthBar.assignBoss( rogue );
        }
    ...
    protected void createItems() {
        ...
        Item s=new Shuriken().quantity(3);
        s.identify();
        do {
            pos = randomPrisonCellPos();
        } while (solid[pos]);
    ...
    public static class DiedTengu extends NPC {
        ...
        public boolean interact(Char c) {
            Dungeon.level.seal();
            Music.INSTANCE.play(Assets.Music.PRISON_BOSS, true);
            ((PrisonNewBossLevel)Dungeon.level).setMapArena(pos);
