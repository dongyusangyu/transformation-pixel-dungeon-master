...
public class Chasm implements Hero.Doom {
	...
	public static void heroLand() {
		...
		}else if(hero.pointsInTalent(Talent.DROP_RESISTANT)==2){
			//Buff.affect( hero, Bleeding.class).set( Math.round(hero.HT / (6f + (6f*(hero.HP/(float)hero.HT)))), Chasm.class);
		}else{
		...
		hero.damage( Math.max( hero.HP / 2, Random.NormalIntRange( hero.HP / 2, hero.HT / 4 )), new Chasm() , DamageTag.PHYSICAL, DamageTag.NO_ARMOR);
	}
