...
public class ShopRoom extends SpecialRoom {
	...
	protected ArrayList<Item> generateItems() {
		...
		itemsToSpawn.add( Generator.randomUsingDefaults( Generator.Category.POTION ) );
		if(Dungeon.isChallenged(Challenges.NO_SCROLLS) && Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT)){
		...
		if (Dungeon.hero != null
				&& Dungeon.hero.hasTalent(Talent.TRANQUIL_TINCTURE)){
			for (int i = 0; i < Dungeon.hero.pointsInTalent(Talent.TRANQUIL_TINCTURE) + 1; i++){
				itemsToSpawn.add( new Tincture() );
			}
		}
