...
public class HighGrass {
	...
	public static void trample( Level level, int pos ) {
		...
		if (level.map[pos] == Terrain.FURROWED_GRASS){
			...
			if(hero.pointsNegative(Talent.THORNS_SPRANG)>Random.Int(10) && ch instanceof Hero){
				Buff.affect(ch, Bleeding.class).set(1);
			}
			if(hero != null && hero.pointsNegative(Talent.GRASS_MOB)>Random.Int(10)){
			...
			if (ch != null) {
				...
				if (ch instanceof Hero && ((Hero) ch).hasTalent(Talent.NATURES_BOUNTY)){
					...
					if (berriesAvailable > 0) {
						...
						if (Dungeon.scalingDepth() > targetFloor) droppingBerry = Random.Int(10) == 0;
						else if (Dungeon.scalingDepth() == targetFloor) droppingBerry = Random.Int(30) == 0;
						else if (Dungeon.scalingDepth() < targetFloor) droppingBerry = Random.Int(90) == 0;
			...
			if (naturalismLevel >= 0) {
				...
				if(naturalismLevel>0 && hero.hasTalent(Talent.JOURNEY_NATURE)){
					lootChance*=1.5f;
				}

				// absolute max drop rate is ~1/6.5 with footwear of nature, ~1/18 without
				lootChance *= PetrifiedSeed.grassLootMultiplier();
				if(Dungeon.isChallenged(Challenges.NO_HERBALISM) && Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT)){
					lootChance *= 0.5f;
				}
