...
public class Badges {
	...
	public enum BadgeType {
        ...
        SECRET //unlocked for the save profile only, usually over multiple runs
	}
	public boolean meta;
	...
	public enum Badge {
        ...
        UNLOCK_PRINCESS                ( copper+10 ),
        UNLOCK_FRIAR                ( copper+11 ),
	...
	static {
		...
		victoryClassBadges.put(HeroClass.PRINCESS, Badge.VICTORY_PRINCESS);
        victoryClassBadges.put(HeroClass.FRIAR, Badge.VICTORY_FRIAR);
	}
	...
	static {
		...
		thirdBossSubclassBadges.put(HeroSubClass.RUNEMAGE, Badge.BOSS_SLAIN_3_RUNEMAGE);
		thirdBossSubclassBadges.put(HeroSubClass.COMBATMASTER, Badge.BOSS_SLAIN_3_COMBATMASTER);
	...
	public static void validateDM400Unlock(){
		if (!isUnlocked(Badge.UNLOCK_DM400)){
			displayBadge( Badge.UNLOCK_DM400 );
		}
	}
    public static void validatePrincessUnlock(){
        if (!isUnlocked(Badge.UNLOCK_PRINCESS)){
            displayBadge( Badge.UNLOCK_PRINCESS );
        }
    }
    public static void validateFriarUnlock(){
        if (!isUnlocked(Badge.UNLOCK_FRIAR)){
            displayBadge( Badge.UNLOCK_FRIAR );
        }
    }
	...
	private static void displayBadge( Badge badge ) {
		...
		if (badge == null || (badge.type != BadgeType.JOURNAL && !Dungeon.customSeedText.isEmpty()) || Dungeon.isChallenged(Challenges.TEST_MODE)   ) {
			return;
		}
	...
	public static String showCompletionProgress( Badge badge ){
		...
		if (badge == Badge.BOSS_SLAIN_1_ALL_CLASSES){
			for (HeroClass cls : HeroClass.values()){
                if(cls==HeroClass.RATKING){break;}
				result += "\n";
			...
			for (HeroSubClass cls : HeroSubClass.values()){
                if(cls.is(HeroSubClass.KING)){break;}
				if (cls.is(HeroSubClass.NONE)) continue;
				result += "\n";
