...
import static com.shatteredpixel.shatteredpixeldungeon.items.EquipableItem.AC_UNEQUIP;
...
public class Dungeon {
	...
	public static int talent_item;
	public static int eat_item;
	...
	public static void reinit() {
		...
		ArrayList<Item> allItems = (ArrayList<Item>) hero.belongings.backpack.items.clone();
	...
	public static Level newLevel() {
		...
		if (curriculumLevel != null) {
			...
			switch (depth) {
				...
				case 10:
					...
					random.pushGenerator(seed+1000000L);
					if(random.Int(2)==0){
						level = new PrisonBossLevel();
					}else {
						level = new PrisonNewBossLevel();
					}
					break;
				case 11:
				case 12:
		...
		if (!(level instanceof DeadEndLevel)) {
			...
			if (depth > Statistics.deepestFloor && branch == 0) {
				...
				if(hero.pointsNegative(Talent.USURY)>0){
					int pp=100+50*hero.pointsNegative(Talent.USURY);
					gold -=pp;
					GLog.w(Messages.get(Talent.USURY,Talent.USURY.name()+".interest",pp));
				}
				if (hero.pointsInTalent(Talent.TREASURE_SENSE) > Random.Int(6)) {
	...
	public static void switchLevel( final Level level, int pos ) {
		...
		if(!hero.buffs( Light.class ).isEmpty() && hero.viewDistance == Light.DISTANCE && Dungeon.isChallenged(Challenges.HARSH_ENVIRONMENT) && Dungeon.isChallenged(Challenges.DARKNESS)){
			hero.viewDistance = 4;
	...
	public static void loadGame( int save, boolean fullLoad ) throws IOException {
		...
		talent_item = bundle.getInt(TALENT_ITEM);
        ...
        if (bundle.contains(DRONEVISON)){
            droneVison = bundle.getBoolean( DRONEVISON );
        }else{
            droneVison = true;
        }
