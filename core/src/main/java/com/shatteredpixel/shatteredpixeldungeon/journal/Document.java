...
public enum Document {
	...
	BACKS(ItemSpriteSheet.BACKS_PAGE, true),
	HERO(ItemSpriteSheet.HERO_PAGE, true);
	...
	public Image pageSprite(String page){
		if (page.isEmpty() || !isPageFound(page) || this != ADVENTURERS_GUIDE){
			...
			switch (page){
				...
				case "tran":
					return new ItemSprite( ItemSpriteSheet.SCROLL_SUBLITION );
				case "tran1":
					return new ItemSprite( ItemSpriteSheet.SCROLL_META );
			}
		}
	}
	...
	static {
		...
		ADVENTURERS_GUIDE.pagesStates.put("tran",              NOT_FOUND);
		ADVENTURERS_GUIDE.pagesStates.put("tran1",             NOT_FOUND);
		...
		BACKS.pagesStates.put("imp",                 debug ? READ : NOT_FOUND);
        ...
        HERO.pagesStates.put("princess",                 debug ? READ : NOT_FOUND);
