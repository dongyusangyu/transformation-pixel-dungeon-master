...
public class ScrollOfDebug extends TestItem {
    public static final String AC_EXECUTE = "EXECUTE";

    {
        image = ItemSpriteSheet.SCROLL_HOLDER;
        defaultAction = AC_EXECUTE;
    }
    ...
    private enum Command {
        ...
        String key(String suffix) {
            return "cmd_" + name().toLowerCase(Locale.ENGLISH) + "_" + suffix;
        }

        String message(String suffix, String fallback) {
            String key = key(suffix);
            return Messages.canget(ScrollOfDebug.class, key) ? Messages.get(ScrollOfDebug.class, key) : fallback;
        }
        ...
        String note(int index) {
            return message("note" + (index + 1), notes[index]);
        }
        ...
        String fullDocumentation(PackageTrie trie, boolean showClasses) {
            ...
            if(notes.length > 0) {
                ...
                else for(int i = 0; i < notes.length; i++) documentation += "\n_-_ " + note(i);
            }
            if(showClasses && paramClass != null && !paramClass.isPrimitive() && paramClass != Object.class) {
                documentation += "\n\n_" + Messages.get(ScrollOfDebug.class, "valid_classes") + "_:" + listAllClasses(trie,paramClass);
            }
    ...
    // -- macro logic
    private static final String MACROS = "debug_macros";
    ...
    public static void storeMacrosInBundle(Bundle bundle) {
        Bundle macroBundle = new Bundle();
        for (Map.Entry<String, String> entry : getMacros().entrySet()) {
            macroBundle.put(entry.getKey(), entry.getValue());
        }
        bundle.put(MACROS, macroBundle);
    }

    public static void restoreMacrosFromBundle(Bundle bundle) {
        macros = new HashMap<>();
        if (!bundle.contains(MACROS)) {
            return;
        }
        Bundle macroBundle = bundle.getBundle(MACROS);
        if (macroBundle.isNull()) {
            return;
        }
        for (String key : macroBundle.getKeys()) {
            macros.put(key, macroBundle.getString(key));
        }
    }
    ...
    public String actionName(String action, Hero hero) {
        if (action.equals(AC_EXECUTE)) {
            return Messages.get(this, "ac_execute");
        }
        return super.actionName(action, hero);
    }

    private void openCommandWindow() {
        GameScene.show(new WndTextInput(Messages.get(this, "input_title"), null, lastCommand, 160, false,
                Messages.get(this, "execute"), Messages.get(this, "cancel")) {
            @Override
    ...
    private String[] handleVariables(String[] input) {
        ...
        if (input.length > 0 && input[0].startsWith(Variable.MARKER)) {
            ...
            if (storeLocation.length() == 1) {
                if (input.length > 1)
                    GLog.w(Messages.get(this, "discarded_args"));
                // list them all
                ...
                GameScene.show(new HelpWindow(Messages.get(this, "active_variables") + "\n" + s));
                return null;
            }
        ...
        for (int i = 0; i < input.length; i++) {
            ...
            do {
                ...
                if (placeholders[cur] != -2) {
                    GLog.n(Messages.get(this, "infinite_parameter_loop", loop));
                    return true;
                }
    ...
    public boolean interpret(String... input) {
        ...
        if (command == null) {
            ...
            GLog.w(Messages.get(this, "invalid_command", input[0]));
            return false;
        }

        if(command == Command.CHANGES) {
            GameScene.show(new HelpWindow(Messages.get(this, "changelog")));
        }
            ...
            if (input.length == 1) {
                ...
                if(!macros.isEmpty()) {
                    msg.append("\n_").append(Messages.get(this, "defined_macros")).append("_:");
            ...
            String failureReason =
                    macroExists ? null : // avoid checks if it already exists
                            Command.get(macro) != null ? Messages.get(this, "macro_failure_existing") :
                                    // should I print out the offending part???
                                    !macro.matches("[A-Za-z_][\\w$_]*") ? Messages.get(this, "macro_failure_name")
                                            : null;
            if (failureReason != null) {
                GLog.n(Messages.get(this, "invalid_macro_name", failureReason));
            } else GameScene.show(new WndTextInput(
                    Messages.get(this, "macro_title", input[1]), Messages.get(this, "macro_body"),
                    macroExists ? macros.get(macro) : "",
                    Integer.MAX_VALUE, // ????
                    true, Messages.get(this, "confirm"), Messages.get(this, "cancel")
            ) {
            ...
            if (storedVariable instanceof Integer) {
                ...
                GLog.w(Messages.get(this, "invalid_argument", storedVariable == null ? input[1] : storedVariable));
            } else {
                GameScene.selectCell(new CellSelector.Listener() {
                    ...
                    public String prompt() {
                        return Messages.get(ScrollOfDebug.class, "prompt_warp");
                    }
                });
            }
            return false;
            ...
            if(command == Command.GOTO) {
                if(storedVariable instanceof Integer) {
                    ...
                    GLog.w(Messages.get(this, "invalid_depth", input[1]));
            ...
            if(command == Command.INSPECT || command == Command.USE && input.length == 2) {
                ...
                if(cls != null) {
                    ...
                    GameScene.show(new HelpWindow(
                            Messages.get(this, "inspection_title", input[1])
                                    + message.toString() ));
                    return false;
                }
            }
            ...
            if(command == Command.USE && input.length > 2) {
                if (cls == null) {
                    GLog.w(Messages.get(this, "class_not_found", input[1]));
                    return false;
                }
                Object o =
                ...
                if(!executeMethod(o, cls, input, 2)) {
                    GLog.w(Messages.get(this, "method_not_found", cls, input[2]));
                    return false;
                }
                return true;
            }
            ...
            if (valid) switch (command) {
                    ...
                    if(manualPlace) {
                        GameScene.selectCell(new CellSelector.Listener() {
                            @Override public String prompt() {
                                return Messages.get(ScrollOfDebug.class, "prompt_place_mob", mob.name());
                            }
                            @Override public void onSelect(Integer cell) {
                                ...
                                if(level.findMob(cell) != null
                                    ...
                                    GLog.w(Messages.get(ScrollOfDebug.class, "cannot_place_mob", mob.name()));
                                    return;
                                }
                                mob.pos = cell;
                                GameScene.add(mob);
                                ...
                                GLog.w(Messages.get(ScrollOfDebug.class, "spawned", mob.name()));
                            }
                        });
                        ...
                        spawned--;
                        GLog.w(Messages.get(this, "spawned_count", mob.name(), spawned));
                    }
                    return true;
                case SET:
                    Trap t = (Trap)o;
                    GameScene.selectCell(new CellSelector.Listener() {
                        ...
                        @Override public String prompt() {
                            return Messages.get(ScrollOfDebug.class, "prompt_set_trap");
                        }
                    });
                    ...
                    for(int i=2; i < input.length; i++) {
                        if(input[i].startsWith("--force") || input[i].equalsIgnoreCase("-f")) {
                            ...
                            if(!executeMethod(item,input,i)) {
                                GLog.w(Messages.get(this, "unrecognized_option", input[i]));
                ...
                case AFFECT:
                    ...
                    GameScene.selectCell(new CellSelector.Listener() {
                        @Override public String prompt() {
                            return Messages.get(ScrollOfDebug.class, "prompt_affect_char");
                        }
                        @Override public void onSelect(Integer cell) {
                            Char target;
                            ...
                            if (index < input.length
                                    && (input[index].equalsIgnoreCase("detach")
                                    || input[index].equalsIgnoreCase("remove"))) {
                                Buff existing = target.buff(cls);
                            ...
                            if(index >= input.length)
                            {
                                ...
                                ) GLog.w(Messages.get(ScrollOfDebug.class, "unsupported_method", input[index]));
                            }
                            if(added == null) {
                                added = Buff.affect(target, cls);
                            }
                ...
                case SEED:
                    ...
                    GameScene.selectCell(new CellSelector.Listener() {
                        @Override public String prompt() {
                            return Messages.get(ScrollOfDebug.class, "prompt_seed_blob");
                        }
                        @Override public void onSelect(Integer cell) {
                            if(cell == null) return;
                ...
                GLog.w(Messages.get(this, "entity_not_found", command.paramClass.getSimpleName(), input[1]));
                return false;
            }
        } else {
    ...
    @Override public String desc() {
        StringBuilder builder = new StringBuilder(Messages.get(this, "desc") + "\n\n" + Messages.get(this, "supported_commands"));
        for(Command cmd : Command.values()) builder.append(
                // this should hopefully fit on one line.
                String.format("_- %s_: %s\n", cmd, cmd.summary())
        );
        return builder.append("\n").append(Messages.get(this, "warning"))
                .toString();
    }
    ...
    private static class HelpWindow extends Window {
        private static final int WIDTH_MIN=120, WIDTH_MAX=220, SCROLLBAR_GUTTER=6;
        ...
        HelpWindow(String message) {
            ...
            int maxHeight = Math.max(60, (int)(PixelScene.uiCamera.height * 0.7));
            boolean needScrollPane = height > maxHeight;
            if(needScrollPane) height = maxHeight;
            resize((int)text.width() + (needScrollPane ? SCROLLBAR_GUTTER : 0), height);
            if(needScrollPane) {
                add(scrollPane = new ScrollPane(new Component()) {
                    ...
                    protected void layout() {
                        ...
                        width += SCROLLBAR_GUTTER; // keep text clear of the scrollbar.
                        super.layout();
                    }
                });
                scrollPane.setSize(width + SCROLLBAR_GUTTER, height);
            }
            else {
                add(text);
            }
