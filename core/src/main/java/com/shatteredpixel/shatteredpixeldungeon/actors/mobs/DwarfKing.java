...
public class DwarfKing extends Mob {
			...
			if (Dungeon.isChallenged(Challenges.STRONGER_BOSSES)){
				...
				if (summonsMade < 6){
					...
					for(Mob m:Dungeon.level.mobs.toArray( new Mob[0] )){
						if((m instanceof DKWarlock ) || (m instanceof DKMonk ) || (m instanceof DKGhoul ) || (m instanceof DKGolem )){
							n++;
						}
					}
					if(n==0){
	...
	public void notice() {
		...
		if (!BossHealthBar.isAssigned()) {
			BossHealthBar.assignBoss(this);
            if(hero!=null && hero.heroClass== HeroClass.PRINCESS){
	...
	public void damage(int dmg, Object src) {
		...
		if (isInvulnerable(src.getClass())){
			...
			return;
		}else if(phase == 1 && !(src instanceof Viscosity.DeferedDamage) && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) && Dungeon.isChallenged(Challenges.HARSH_ENVIRONMENT)){
			if (dmg >= 0) {
				Viscosity.DeferedDamage deferred = Buff.affect( this, Viscosity.DeferedDamage.class );
				deferred.prolong( dmg );
	...
	public void die(Object cause) {
		...
		if (Dungeon.level.solid[pos]){
			...
			if(!Statistics.subLimation[3]){
				Dungeon.level.drop(new ScrollOfSublimation().type("DWARFKING"), pos + Dungeon.level.width()).sprite.drop(pos);
