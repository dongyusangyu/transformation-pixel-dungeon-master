...
public abstract class ClericSpell {
	...
	public static ArrayList<ClericSpell> getSpellList(Hero cleric, int tier){
		...
		if (tier == 1) {
			...
			if (cleric.hasTalent(Talent.ASCENSION_CURSE)) {
				spells.add(Ascension_Curse.INSTANCE);
			}
			if (cleric.hasTalent(Talent.SHEPHERD_INTENTION)) {
				spells.add(Shepherd_Intention.INSTANCE);
			}

			if (cleric.hasTalent(Talent.SILVER_LANGUAGE)) {
				spells.add(Silver_Language.INSTANCE);
			}
			if(cleric.hasTalent(Talent.TRAITOROUS_SPELL)) {
				spells.add(Traitorous_Spell.INSTANCE);
			}
			...
			if(cleric.hasTalent(Talent.DIVINE_PROTECTION)){
				spells.add(Divine_Protection.INSTANCE);
			}
			if(cleric.hasTalent(Talent.CONVERSION_HOLY)){
				spells.add(Conversion_Holy.INSTANCE);
			}
			if(cleric.hasTalent(Talent.GENESIS)){
				spells.add(Genesis.INSTANCE);
			}
			if (cleric.hasTalent(Talent.INDULGENCE)) {
				spells.add(Indulgence.INSTANCE);
			}
			if (cleric.hasTalent(Talent.SPRINT_SPELL)){
				spells.add(Sprint_Spell.INSTANCE);
			}
			...
			if (cleric.hasTalent(Talent.PURIFYING_EVIL)){
				spells.add(Purifying_Evil.INSTANCE);
			}
			if (cleric.hasTalent(Talent.DIVINE_STORM)){
				spells.add(Divine_Storm.INSTANCE);
			}
