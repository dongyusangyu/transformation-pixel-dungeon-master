...
public class ElementalStrike extends ArmorAbility {
	...
	static {
        ...
        effectTypes.put(CorrosionEnchanted.class,      MagicMissile.CORROSION_CONE);
		...
		hero.sprite.attack(target, new Callback() {
			...
			public void call() {
				...
				if (enemy != null){
					...
					if (!enemy.isAlive() && finalEnchantment instanceof Sweeping) {
						reactivate = true;
					}
				}
	...
	private void perCharEffect(ConeAOE cone, Hero hero, Char primaryTarget, Weapon.Enchantment ench) {
		...
		if (ench == null) {
			for (Char ch : affected){
				ch.damage(Math.round(powerMulti* Hero.heroDamageIntRange(6, 12)), ElementalStrike.this, DamageTag.MAGICAL);
			}
		...
		//*** Kinetic ***
			...
			if (storedKineticDamage > 0) {
				for (Char ch : affected) {
					if (ch != primaryTarget) {
						ch.damage(Math.round(storedKineticDamage * 0.4f * powerMulti), ench, DamageTag.PHYSICAL);
					}
				}
		...
		//*** Projecting ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget) {
					ch.damage(Math.round(hero.damageRoll() * 0.3f * powerMulti), ench, DamageTag.PHYSICAL);
				}
			}
		...
		//*** Grim ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget) {
					...
					if (Random.Float() < chance*powerMulti){
						ch.damage( ch.HP, Grim.class , DamageTag.MAGICAL);
		...
		//*** Polarized ***
			...
			for (Char ch : affected){
				if (Random.Float() < 0.5f*powerMulti){
					ch.damage(Hero.heroDamageIntRange(24, 36), ElementalStrike.this, DamageTag.MAGICAL);
				}
			}
		...
		//*** Friendly ***
			...
			//*** Heavy ***
		}else if (ench instanceof Heavy){
			for (Char ch : affected){
				if (Random.Float() < 0.5f*powerMulti){
					Buff.affect(ch, Weakness.class, 6f);
				}
			}
		}
			//*** Sweeping ***
		else if (ench instanceof Sweeping) {
			for (Char ch : affected) {
				ch.damage(Math.round(powerMulti * Hero.heroDamageIntRange(6, 12)), ench, DamageTag.PHYSICAL);
			}
		}else if (ench instanceof CorrosionEnchanted){
            for (Char ch : affected){
                Buff.affect(ch, Corrosion.class).set(6f*powerMulti, (int)(2*powerMulti), null);
            }
        }
