...
public class Burning extends Buff implements Hero.Doom {
	...
	public void detach() {
		...
		if(target instanceof Hero && hero.hasTalent(Talent.NIRVANA)){
			if (hero.HP < hero.HT ) {
				hero.sprite.showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(Math.min(Math.round(hero.lvl/2), hero.HT - hero.HP)), FloatingText.HEALING);
				hero.HP = Math.min(hero.HT, hero.HP + Math.round(hero.lvl/2));
			}
		}
	...
	public boolean act() {
		...
		if (acted && Dungeon.level.water[target.pos] && !target.flying){
			if (target !=null && target instanceof Hero && hero.pointsNegative(Talent.UPDRAFT)>0){
				Buff levitation=hero.buff(Levitation.class);
				if(levitation!=null){
					levitation.detach();
				}
			}
			...
			if(target !=null && target instanceof Hero && hero.pointsNegative(Talent.UPDRAFT)>0 && hero.buffs(Levitation.class).isEmpty() && left>1){
				Buff.affect(hero, Levitation.class,left);
			}
			...
			if (target instanceof Hero
				...
				int chance = burnIncrement - 3;
				if(hero.pointsNegative(Talent.BONE_FIRE)==2){
					chance *= 2;
				}
				if (Random.Int(3) < chance){
					burnIncrement = 0;
