...
public class Invisibility extends FlavourBuff {
	...
	public boolean attachTo( Char target ) {
		if (super.attachTo( target )) {
			...
			if(hero!=null && target==hero && hero.pointsInTalent(Talent.INVISIBILITY_SHADOWS)>0 && hero.buffs(Talent.InvShaCooldown.class).isEmpty()
			&& hero.sprite!=null){
	...
	public void detach() {
		...
		if(target==hero && hero.hasTalent(Talent.PHANTOM_STEP)){
			Buff.affect(hero, Haste.class,1+2*hero.pointsInTalent(Talent.PHANTOM_STEP));
	...
	public static void dispel(Char ch){
		...
		if(ch==hero && hero.hasTalent(Talent.CICADA_DANCE) && hero.buff(Talent.EnemyDies.class)!= null && hero.subClass==HeroSubClass.ASSASSIN){
