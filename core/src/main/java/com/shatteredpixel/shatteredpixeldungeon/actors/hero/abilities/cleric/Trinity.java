...
public class Trinity extends ArmorAbility {
					...
					btnBody = new RedButton(Messages.get(WndUseTrinity.class, "body",
						...
						protected void onClick() {
							...
							if (duplicatesBodyFormEffect((Weapon) hero.belongings.weapon(), hero.belongings.armor(),
								...
								Buff.prolong(hero, BodyForm.BodyFormBuff.class, BodyForm.duration()).setEffect(bodyForm);
	...
	public static class WndItemtypeSelect extends Window {
		...
		public WndItemtypeSelect(HolyTome tome, ClericSpell spell) {
			...
			resize( WIDTH, (int)pos+80+MARGIN);
			float x = 0;
			...
			pane = new ScrollPane(new Component()){
                public void onClick(float x, float y) { WndItemtypeSelect.this.onClick(x, y);}
            };;
			add(pane);
