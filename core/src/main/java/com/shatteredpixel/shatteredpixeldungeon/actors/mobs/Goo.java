...
public class Goo extends Mob {
	...
	private boolean[] summon = new boolean[5];
	...
	public int attackSkill( Char target ) {
		...
		if(pumpedUp >0 && Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT) && Dungeon.isChallenged(Challenges.STRONGER_BOSSES)){
			attack = INFINITE_ACCURACY;
		}
		return attack;
	}
	...
	public boolean act() {
		...
		if (!flying && Dungeon.level.water[pos] && HP < HT) {
			int maxHeakInc = 3;
			if(Dungeon.isChallenged(Challenges.HARSH_ENVIRONMENT) && Dungeon.isChallenged(Challenges.STRONGER_BOSSES)){
				maxHeakInc = 4;
			}
			HP += healInc;
			Statistics.qualifiedForBossChallengeBadge = false;
			...
			if (Dungeon.isChallenged(Challenges.STRONGER_BOSSES) && healInc < maxHeakInc) {
				healInc++;
			}
			if (HP*2 > HT) {
	...
	public void die( Object cause ) {
		...
		if(!Statistics.subLimation[0]){
			Dungeon.level.drop( new ScrollOfSublimation().type("GOO"), pos ).sprite.drop();
	...
	private final String SUMMON = "summon";
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		if (bundle.contains( SUMMON ))summon = bundle.getBooleanArray(SUMMON);
		else                                summon = new boolean[5];
