...
public class Ooze extends Buff {
	...
	public boolean act() {
		...
		//史莱姆
		if (acted && Dungeon.level.water[target.pos] && !target.flying && !hero.hasTalent(Talent.POTENT_OOZE)){
			detach();
		} else if (target.isAlive()) {
			if(target instanceof Hero && hero.subClass.is(HeroSubClass.DARKSLIME)){
				detach();
				return true;
			}
			if(hero.hasTalent(Talent.POTENT_OOZE) && acted && Dungeon.level.water[target.pos] && !target.flying){
				left -=4 - 1*hero.pointsInTalent(Talent.POTENT_OOZE);
			}
			acted = true;
		...
		if (Dungeon.level.water[target.pos] && !target.flying && !hero.hasTalent(Talent.POTENT_OOZE)){
			detach();
		}
		return true;
