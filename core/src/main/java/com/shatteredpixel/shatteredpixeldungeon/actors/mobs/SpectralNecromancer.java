...
public class SpectralNecromancer extends Necromancer {
	...
	public void rollToDropLoot() {
		...
		if (Dungeon.hero.lvl > mlvl + 2) return;
	...
	public void summonMinion() {
		if (Actor.findChar(summoningPos) != null) {
			...
			if (pushPos != pos) {
				...
				if (blocker.alignment != alignment){
					blocker.damage( Random.NormalIntRange(2, 10), new SummoningBlockDamage() , DamageTag.PHYSICAL, DamageTag.NO_ARMOR);
		...
		Wraith wraith = Wraith.spawnAt(summoningPos, Wraith.class, alignment);
		if (wraith == null){
			spend(TICK);
			return;
		}
