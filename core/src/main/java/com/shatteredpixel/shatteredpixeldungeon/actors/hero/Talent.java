...
public enum Talent {
    ...
    //PRINCESS
    ENCHANT_MEAL(544),MANA_SENSE(545),RUNE_BLADE(546),ARCANE_SHIELD(547),
    ROYAL_MEAL(548),RUNE_EXPERT(549),MORE_RING(550),MANA_WREATH(551),MAGIC_ARROW(552),
    STR_RUNE(553,3),RING_BOND(554,3),
    DARKMARK(555,3),RUNE_BLAST(556,3),RUNE_SURGE(557,3),
    FLUENT(558,3),STANCE_MASTERY(559,3),INELEMENT(560,3),
    SPEEDUP(561,4),RUN_ATTACK(562,4),DELAY_TACTIC(563,4),
    ELEMENT_CURSE(564,4),AFFLICTED_ILLNESS(565,4),SPREAD_PAIN(566,4),
    KING_GAZE(567,4),KING_POWER(568,4),KING_MIGHT(569,4),
	...
	//DM300
    THUNDER_STRIKE(166),DIRECTIONAL_COLLAPSE(167),FASTING(168,2),
	//DwaefKing
	...
	ENGINEER_REFIT(188,2),WEIRD_THROW(189,2),THRID_HAND(190,2),WATER_ATTACK(191,2),
	...
	MIRACLE_ALCHEMY(477),HUNGRY_GHOST(478),UNABLE_REST(479),ELITE_RECRUIT(507,4),PRIM_ACCU(508,4),
	SOUL_CONTRACT(509,4),CURSEDMAN(511),SHADOW_KILLER(512),KILL_SPREE(513),SEAOFPEOPLE(514),PHANTOM_STEP(515),
	USURY(516),ARMED_UPRISING(517),DUNGEON_HERO(518),EXP_SOLID(519), WEALTH_STATUE(520,3),
	SWEET_SLEEP(521),LIGHT_SHOCK(522),BOOZY(523),STEAM_BEAN(524),ALCHEMY_ACCIDENT(525),SHELL_CRY(526),
	MAL_CURSE(527),OVERLOAD_CHARGE(528,2),GAS_SPURT(529,2),
	//Friar T1
	TRANQUIL_TINCTURE(576), RAVENS_EYE(577), CRYSTAL_GUNPOWDER(578), BULWARK_GREATSHIELD(579),
	//Friar T2
	WHISPERING_MEAL(580), LIQUID_BARRIER(581), FAST_RELOAD(582), OTHERWORLD_BANE(583), BULLSEYE(584),
	//Friar T3
	MIDAS_TOUCH(585, 3), HUMAN_GLORY(586, 3),
	//Alchemist T3
	ALCHEMY_SHIELD(587, 3), CLOSE_BLAST(588, 3), CONSERVATION(589, 3),
	//Reason Dose armor ability
	HEART_DOSE(593, 4), SHIELD_DOSE(594, 4), CALM_DOSE(595, 4),
	//Friar armor ability
	CONCENTRATED_ESSENCE(596, 4), REAGENT_ENHANCEMENT(597, 4), REUSE_REAGENT(598, 4),
	PIOUS_FAITH(599, 4), SUPREME_BLESSING(600, 4), HATRED_OF_EVIL(601, 4),
	//Pious T3
	NEVER_COMPROMISE(590, 3), REWIND_TIME(591, 3), BRIGHT_WARRIOR(592, 3),
	// Ratking T1
	ROYAL_PRIVILEGE(608), ROYAL_INTUITION(609), KINGS_WISDOM(610), NOBLE_CAUSE(611),
	// Ratking T2
	RK_ROYAL_MEAL(612), RESTORATION(613), POWER_WITHIN(614), KINGS_VISION(615), PURSUIT(616),
	// RatKing T3
	RK_BERSERKER(617,3), RK_GLADIATOR(618,3), RK_BATTLEMAGE(619,3), RK_WARLOCK(620,3),
	RK_ASSASSIN(621,3), RK_FREERUNNER(622,3), RK_SNIPER(623,3), RK_WARDEN(624,3),
	RK_CHAMPION(625,3), RK_MONK(626,3), RK_PRIEST(627,3), RK_PALADIN(628,3),
	RK_WATERSLIME(651,3), RK_DARKSLIME(652,3),
	RK_TATTEKI_NINJA(653,3), RK_NINJA_MASTER(654,3), RK_AT400(655,3), RK_AU400(656,3),
	RK_RUNEMAGE(657,3), RK_COMBATMASTER(658,3),
    ;
	...
	public String desc(boolean metamorphed){
		...
		return !Messages.canget(this.getClass(), name() + ".comment") ? desc : desc + "\n\n" + comment;
	}
	...
	private static boolean isPhysicalAttack( Hero hero ){
		return hero != null && hero.belongings.attackingWeapon() != null;
	}

	private static int crystalGunpowderBonus( Hero hero ){
		if (hero == null || !hero.hasTalent(CRYSTAL_GUNPOWDER)){
			return 0;
		}
		return Math.min(2 * hero.pointsInTalent(CRYSTAL_GUNPOWDER), Dungeon.energy * hero.pointsInTalent(CRYSTAL_GUNPOWDER));
	}
	...
	public static float eatingTime( Hero hero, Item foodSource, float time ){
		if (hero != null && hero.hasTalent(WHISPERING_MEAL)){
			return foodSource instanceof com.shatteredpixel.shatteredpixeldungeon.items.food.Tincture ? 0f : Math.min(time, 1f);
		}
		return time;
	}

	private static void procWhisperingMeal( Hero hero ){
		if (hero == null || !hero.hasTalent(WHISPERING_MEAL)){
			return;
		}
		float duration = hero.pointsInTalent(WHISPERING_MEAL) == 1 ? 3f : 5f;
	...
	public static int bullseyeDamageBonus( Hero hero, Char target ){
		if (hero != null && target != null && hero.hasTalent(BULLSEYE) && target.alignment != hero.alignment){
			return hero.pointsInTalent(BULLSEYE) == 1 ? 3 : 5;
		}
		return 0;
	}
	...
	public static int onAttackProcBonus( Hero hero, Char enemy){
		int dmg =0;
		if (isPhysicalAttack(hero)){
			dmg += crystalGunpowderBonus(hero);
		}
	...
	public static int onDamage(  int dmg, Object src  ){
		if (hero != null){
			...
			if (dmg >= hero.HP && triggerDyingWill(dmg)){
	...
	private static boolean triggerDyingWill(int dmg){
		if (hero == null || !hero.hasTalent(NEVER_COMPROMISE) || !hasSufferingOrVirtue(hero)){
			return false;
		}
	...
	public static int beforeSufferingReasonLoss(Hero hero, int lose){
		...
		if (hasSuffering(hero) && r.reason >= 20 && r.reason - lose < 20){
			...
			Buff.affect(hero, RewindTimeUsed.class);
		}
		return lose;
	}

	public static class RewindTimeUsed extends Buff {
		{
			type = buffType.POSITIVE;
		}
    ...
    public static   int onWandDamage(Wand wand, int dmg){
	...
	public static void onEmenyDie(Char emeny,  Object cause ){
        ...
        if(hero.heroClass==HeroClass.FRIAR){
			com.shatteredpixel.shatteredpixeldungeon.items.weapon.melee.RitualDagger ritualDagger = hero.belongings.getItem(com.shatteredpixel.shatteredpixeldungeon.items.weapon.melee.RitualDagger.class);
			if (ritualDagger != null){
				ritualDagger.onKill(hero);
			}
			int virtueKillExtend = hero.subClass.is(HeroSubClass.PIOUS) && hero.hasTalent(BRIGHT_WARRIOR) ? 3 + hero.pointsInTalent(BRIGHT_WARRIOR) : 3;
	...
	static {
		...
		Collections.addAll(typeTalent.get(0).get(EFFECT),HEARTY_MEAL,BACKUP_BARRIER,PROTECTIVE_SHADOWS,NATURES_AID,AGGRESSIVE_BARRIER,
				...
				COLLECTION_GOLD,MARK_MEAL,BOOZY,ENCHANT_MEAL);
		Collections.addAll(typeTalent.get(0).get(RESOURCE),CACHED_RATIONS,NATURES_BOUNTY,THRID_HAND,MORE_TALENT,NOVICE_BENEFITS,ILLUSION_FEED,
				...
				MORE_FAVORS,INVISIBILITY_SHADOWS,OVER_CODE,DUNGEON_HERO,TRANQUIL_TINCTURE);
		...
		Collections.addAll(typeTalent.get(0).get(ASSIST),VETERANS_INTUITION,IRON_WILL,SCHOLARS_INTUITION,THIEFS_INTUITION,
                ...
                FALSEHOOD_POWER,SPIDER_SENSE,TBM,MANA_SENSE,ARCANE_SHIELD,RAVENS_EYE,BULWARK_GREATSHIELD);
		...
		Collections.addAll(typeTalent.get(1).get(ATTACK),LETHAL_MOMENTUM,IMPROVISED_PROJECTILES,STRONG_THROW,JUSTICE_PUNISH,GHOLL_WITCHCRAFT,
				...
				LETHAL_HASTE,NINJA_SOCIAL,ROCKET_FIST,SWEET_SLEEP,MAGIC_ARROW,BULLSEYE);
		Collections.addAll(typeTalent.get(1).get(MAGIC),ENERGIZING_MEAL,INSCRIBED_POWER,ARCANE_VISION,SHIELD_BATTERY,BURNING_CURSE,
				WULEI_ZHENGFA,MAGIC_GIRL,ABYSSAL_GAZE,QUANTUM_HACKING,EAT_MIND,LIGHT_SHOCK,GAS_SPURT);
		Collections.addAll(typeTalent.get(1).get(EFFECT),IRON_STOMACH,LIQUID_WILLPOWER,MYSTICAL_MEAL,INSCRIBED_STEALTH,INVIGORATING_MEAL,
				...
				OVER_MEAL,RECOVER_CHARGE,ROYAL_MEAL,OVERLOAD_CHARGE,WHISPERING_MEAL,LIQUID_BARRIER,OTHERWORLD_BANE);
		Collections.addAll(typeTalent.get(1).get(RESOURCE),WAND_PRESERVATION,ROGUES_FORESIGHT,PRECIOUS_EXPERIENCE,MORE_CHANCE,WANT_ALL,
				SEED_RECYCLING,INSTANT_REFINING,FAST_BREAK,KEBI,MIRACLE_ALCHEMY,EXP_SOLID,RUNE_EXPERT);
		...
		Collections.addAll(typeTalent.get(1).get(ASSIST),WIDE_SEARCH,SILENT_STEPS,REJUVENATING_STEPS,HEIGHTENED_SENSES,DURABLE_PROJECTILES,
				...
				NATURAL_AFFINITY,QUALITY_ABSORPTION,QUICK_SEARCH,BODY_REINFORCE,EFFICIENT_ORDER,MORE_RING,MANA_WREATH,FAST_RELOAD);
		...
		Collections.addAll(typeTalent.get(2).get(RESOURCE),TRAP_MASTER,GOLD_FORMATION,DOUBLE_TRINKETS,RETURNING_HONOR,WEAPON_MAKE,SECRET_STASH,
				PYROMANIAC,BIRTHDAY_GIFT,COLLECT_PLANTS,TREASURE_SENSE,CHANGQI_BOOKSTORE,HARDWARE_ELEMENTS,WEALTH_STATUE,STR_RUNE,
				MIDAS_TOUCH);
		...
		Collections.addAll(typeTalent.get(2).get(ASSIST),HOLD_FAST,STRONGMAN,LIGHT_CLOAK,BEHEST,AFRAID_DEATH,HERO_NAME,HOMETOWN_CLOUD,WIDE_KNOWLEDGE,
				CONCEPT_GRID,ACTIVE_MUSCLES,SEA_WIND,BEYOND_LIMIT,EXTREME_REACTION,HOLY_FAITH,ORIGINAL_MONSTER,LIGHT_BOX,QUICK_TOOL,RING_BOND,
				HUMAN_GLORY);
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents, LinkedHashMap<Talent, Talent> replacements ,LinkedHashMap<Talent, String> sublimation){
		...
		Collections.addAll(tierTalents, AQUATIC_RECOVER,PUMP_ATTACK,OOZE_ATTACK,STRONGEST_SHIELD,COMBO_PACKAGE,BREAK_ENEMY_RANKS,
				SURPRISE_THROW,SMOKE_MASK,RUSH,SHADOW_KILLER,KILL_SPREE,SEAOFPEOPLE,PHANTOM_STEP);
		...
		tierTalents.clear();
		Collections.addAll(tierTalents,FASTING,THUNDER_STRIKE,DIRECTIONAL_COLLAPSE,
