...
public class YogDzewa extends Mob implements MagicalRangedAttack {
			...
			if (!Dungeon.hero.rooted) {
				...
				for (Char ch : affected) {
					...
					if (ch == Dungeon.hero) {
                        ...
                        Reason.loseReason(hero,Dungeon.isChallenged(Challenges.STRONGER_BOSSES)?50:25);
                        if(hero.heroClass==HeroClass.FRIAR){
                            Buff.extend(hero, Panic.class,20);
			...
			while (summonCooldown <= 0){
				...
				if (spawnPos != -1) {
					...
					if(Dungeon.isChallenged(Challenges.STRONGER_BOSSES) && Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT)){
						Buff.affect(summon, Invulnerability.class,20);
					}
	...
	public void damage(int dmg, Object src, DamageTag... damageTags) {
		...
		if (phase < 4 && HP <= HT - 300*phase){
			...
			phase++;
            if(hero.heroClass==HeroClass.FRIAR){
                Buff.extend(hero, Panic.class,25);
                Sample.INSTANCE.play( Assets.Sounds.PANIC, 1, 1, Random.Float( 0.9f, 1.1f ) );
            }
	...
	public void addFist(YogFist fist){
		...
		if(Dungeon.isChallenged(Challenges.STRONGER_BOSSES) && Dungeon.isChallenged(Challenges.HARSH_ENVIRONMENT)){
			Buff.affect(fist, Invulnerability.class,15);
		}
	...
	public void die( Object cause ) {
        ...
        Badges.validateFriarUnlock();
		Badges.validateTakingTheMick(cause);

		Dungeon.level.unseal();
		if(!Statistics.subLimation[4]){
			Dungeon.level.drop( new ScrollOfSublimation().type("YOG"), pos ).sprite.drop();
			Statistics.subLimation[4] = true;
		}
		super.die( cause );
	...
	public void notice() {
		if (!BossHealthBar.isAssigned()) {
            ...
            if(hero!=null && hero.heroClass== HeroClass.FRIAR){
                yell(Messages.get(this, "notice1"));
            }else{
