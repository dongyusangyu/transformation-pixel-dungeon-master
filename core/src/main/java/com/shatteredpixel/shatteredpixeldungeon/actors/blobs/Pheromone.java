...
public class Pheromone extends Blob {
    ...
    private ChaosDisciples.SuccBoss source;
    @Override
    protected void evolve() {
        ...
        if (volume == 0){
            ...
            for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
                if(mob instanceof ChaosDisciples.SuccBoss){
                    source = (ChaosDisciples.SuccBoss)mob;
                    break;
                }
            }
                ...
                if (cur[cell] > 0 && (ch = Actor.findChar( cell )) != null) {
                    if (!ch.isImmune(this.getClass())) {
                        ...
                        if(ch instanceof Hero || ch.alignment == hero.alignment){
                            if(source!=null){
                                Buff.affect(ch, Charm.class,10).object=source.id();
                            }else{
