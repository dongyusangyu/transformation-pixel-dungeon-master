...
public class Blacksmith extends NPC {
	...
	public boolean interact(Char c) {
		...
		if(Dungeon.hero.pointsInTalent(Talent.DUNGEON_HERO)==2 && buff(Talent.SuckerPunchTracker.class)==null){
			Dungeon.level.drop(genMidValueConsumable(), Dungeon.hero.pos).sprite.drop();
			Dungeon.level.drop(genMidValueConsumable(), Dungeon.hero.pos).sprite.drop();
			Buff.affect(this,Talent.SuckerPunchTracker.class);
		}else if(Dungeon.hero.pointsInTalent(Talent.DUNGEON_HERO)>0 && buff(Talent.SuckerPunchTracker.class)==null){
		...
		if (!Quest.given) {
			...
			if (Quest.type == Quest.OLD){
                ...
                msg1 += Messages.get(Blacksmith.this, "intro_quest_" + Dungeon.hero.heroClass.name());
                /*
				switch (hero.heroClass){
                    ...
                    case PRINCESS: msg1 += Messages.get(Blacksmith.this, "intro_quest_princess"); break;
                    case FRIAR: msg1 += Messages.get(Blacksmith.this, "intro_quest_friar"); break;
					...
					GameScene.show(new WndQuest(Blacksmith.this, msg1Final) {
						...
						public void hide() {
							...
							if (pick.doPickUp( hero )) {
								GLog.i( Messages.capitalize(Messages.get(hero, "you_now_have", pick.name()) ));
							} else {
	...
	public boolean add( Buff buff ) {
		if(buff instanceof Talent.SuckerPunchTracker){
	...
	public static class Quest {
		...
		public static void complete(){
			...
			int goldValue = 50;
			int maxFavor = 3000;
			if(hero!=null && hero.hasTalent(Talent.MORE_FAVORS)){
				goldValue *= 1+ 0.5f*hero.pointsInTalent(Talent.MORE_FAVORS);
				if(hero.pointsInTalent(Talent.MORE_FAVORS) == 2){
					maxFavor += 1000;
				}
			}
			DarkGold gold = hero.belongings.getItem(DarkGold.class);
			if (gold != null){
				favor += Math.min(maxFavor, gold.quantity()*goldValue);
				gold.detachAll(hero.belongings.backpack);
			}
			...
			favor = Math.min(maxFavor, favor);
			Statistics.questScores[2] += Math.min(3000, favor);
