...
public abstract class Mob extends Char {
	...
	public int attackProc( Char enemy, int damage ) {
		...
		if(hero.pointsNegative(Talent.BAT_SERUM)>0 && !(this instanceof NPC)){
			int reg = Math.min((int)(damage*0.2f*hero.pointsNegative(Talent.BAT_SERUM)),HT-HP);
			if (reg > 0) {
				HP += reg;
	...
	public void die( Object cause ) {
		...
		if(hero.pointsNegative(Talent.UNABLE_REST)>0 && buff(Talent.UableRest.class)==null && (this.properties().contains(UNDEAD) || this.properties().contains(DEMONIC))
				&& !this.properties().contains(BOSS) && !(this instanceof Ghoul)){
			this.HP=1;
		...
		if (hero.isAlive() && !Dungeon.level.heroFOV[pos] ) {
			...
			if(hero.hasTalent(Talent.POWERFUL_CALCULATIONS)){
				hero.HP=Math.min(hero.pointsInTalent(Talent.POWERFUL_CALCULATIONS)+hero.HP,hero.HT);
				hero.sprite.showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(hero.pointsInTalent(Talent.POWERFUL_CALCULATIONS)), FloatingText.HEALING);
			}
		}
