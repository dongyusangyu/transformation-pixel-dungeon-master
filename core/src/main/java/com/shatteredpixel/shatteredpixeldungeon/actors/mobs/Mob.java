...
import static com.shatteredpixel.shatteredpixeldungeon.actors.Char.Property.DEMONIC;
import static com.shatteredpixel.shatteredpixeldungeon.actors.Char.Property.INORGANIC;
import static com.shatteredpixel.shatteredpixeldungeon.actors.Char.Property.UNDEAD;
...
import com.shatteredpixel.shatteredpixeldungeon.items.RedEnvelope;
...
public abstract class Mob extends Char {
	...
	public int attackProc( Char enemy, int damage ) {
		...
		if(hero.pointsNegative(Talent.DUMP_TRUCK)>0 && properties().contains(Property.LARGE)){
			damage*=1+0.5f*hero.pointsNegative(Talent.DUMP_TRUCK);
		}
		if(hero.pointsNegative(Talent.PHASECLAW)==2 && Random.Int(10)==1){
	...
	protected Char chooseEnemy() {
		...
		//We are charmed and current enemy is what charmed us
			...
			newEnemy = true;
		//Current enemy is no longer hostile to us
		} else if (buff(Amok.class) == null && !Actor.isHostile(this, enemy)) {
			newEnemy = true;
		}
		...
		if ( newEnemy ) {
			...
			//if we are one of the special enemy alignments...
			} else if (Actor.isSpecialEnemyAlignment(alignment)) {
				//look for the other special enemy alignments to attack
				for (Mob mob : Dungeon.level.mobs)
					if (Actor.isHostile(this, mob) && fieldOfView[mob.pos]
			...
			//if we are an ally...
				...
				for (Mob mob : Dungeon.level.mobs)
					if (mob.alignment == Alignment.ENEMY
							&& !(mob instanceof DeceptiveHeroTarget) && fieldOfView[mob.pos]
							&& mob.invisible <= 0 && !mob.isInvulnerable(getClass()))
		...
		if (!isInvulnerable(src.getClass())) {
			...
			if (!(src instanceof Corruption) && state != FLEEING) {
				if (state != HUNTING) {
					...
					if ((src instanceof Wand || src instanceof ClericSpell || src instanceof ArmorAbility) && currentHero != null) {
						aggro(currentHero);
						target = currentHero.pos;
					}
				} else {
					if ((src instanceof Wand || src instanceof ClericSpell || src instanceof ArmorAbility) && currentHero != null) {
						recentlyAttackedBy.add(currentHero);
					}
				}
			}
		}
		if (currentHero != null && currentHero.hasTalent(Talent.WELLFED_MEAL) && !currentHero.buffs(WellFed.class).isEmpty()){
			dmg*=1+currentHero.pointsInTalent(Talent.WELLFED_MEAL)*0.15f;
		}
	...
	public void destroy() {
		...
		Hero currentHero = hero;
		if (currentHero != null && currentHero.buff(MindVision.class) != null){
			Dungeon.observe();
		...
		if (currentHero != null && currentHero.isAlive()) {
			...
			if (alignment == Alignment.ENEMY) {
				...
				int exp = currentHero.lvl <= mlvl ? EXP : 0;
				...
				if (currentHero.buff(AscensionChallenge.class) != null &&
						exp == 0 && mlvl > 0 && EXP > 0 && currentHero.lvl < Hero.MAX_LEVEL){
					exp = Math.round(10 * spawningWeight());
				}

				if (exp > 0) {
					currentHero.sprite.showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(exp), FloatingText.EXPERIENCE);
				}
				...
				if (hero.subClass.is(HeroSubClass.NINJA_MASTER)){
	...
	public void die( Object cause ) {
		...
		if(hero.pointsNegative(Talent.UNABLE_REST)>0 && buff(Talent.UableRest.class)==null  && (this.properties().contains(UNDEAD) || this.properties().contains(DEMONIC) )
				&& !this.properties().contains(BOSS) && !(this instanceof Ghoul) && cause != Chasm.class){
			this.HP=1;
			if (cause == Chasm.class){
				Buff.affect(this, Levitation.class,3f);
			}
			Buff.affect(this,Talent.UableRest.class);
			if(hero.pointsNegative(Talent.UNABLE_REST)==2){
		...
		if(!this.buffs(Resurrection.REsurrection.class).isEmpty() && !this.properties().contains(BOSS) && !this.properties().contains(BOSS_MINION) && cause != Chasm.class){
			...
			b.detach();
			this.HP=this.HT;
			if(hero != null && hero.pointsInTalent(Talent.RESURRECTION)==3){
				Buff.affect(this, Bless.class,20);
			}
			return;
		}
		if(!this.buffs(ChampionEnemy.Holy.class).isEmpty() && cause != Chasm.class){
			this.HP=Math.max(this.HT/4,1);
		...
		if(hero != null && hero.hasTalent(Talent.RETURNING_HONOR)){
			EXP+=(int)(EXP*0.25*hero.pointsInTalent(Talent.RETURNING_HONOR));
		}
		...
		if(hero != null && hero.pointsInTalent(Talent.ZHUOJUN_BUTCHER)>Random.Int(10) && this.properties().isEmpty() && (!(this instanceof NPC) && hero.lvl <= mlvl + 2)){
			Dungeon.level.drop(new MysteryMeat(),pos).sprite.drop(pos);
		}
		if(hero != null && hero.hasTalent(Talent.INSTANT_REFINING) && hero.pointsInTalent(Talent.INSTANT_REFINING)+1>Random.Int(5) &&
				buff(AllyBuff.class)==null && this.alignment== Alignment.ENEMY  && hero.lvl <= mlvl + 2){
			Dungeon.energy+=1;
		...
		if (hero != null && hero.isAlive() && !Dungeon.level.heroFOV[pos] ) {
			...
			if(hero.hasTalent(Talent.POWERFUL_CALCULATIONS)){
                hero.heal(hero.pointsInTalent(Talent.POWERFUL_CALCULATIONS));
			}
		}
		...
		if (buff(ChampionEnemy.RandomMiniBoss.class) != null && canDropRandomMiniBossLoot()) {
			...
			switch (Random.Int(3)) {
				...
				default:
					...
					break;
				case 1:
					loot = Generator.random(Generator.Category.MISSILE);
					break;
				case 2:
					loot = Generator.random(Generator.Category.ARMOR);
					break;
			}
			if (loot != null) {
				loot.level(Random.Int(4));
				loot.cursed = false;
				loot.cursedKnown = true;
				Dungeon.level.drop(loot, pos).sprite.drop();
			}
		}
	...
	protected class Sleeping implements AiState {
		...
		protected void awaken( boolean enemyInFOV ){
			...
			if(hero!=null && hero.pointsNegative(Talent.JUMP_FACE_SILICONE)>0){
				Buff.affect(Mob.this, Swiftthistle.TimeBubble.class).reset1(hero.pointsNegative(Talent.JUMP_FACE_SILICONE));
			}
			...
			if (alignment == Alignment.ENEMY && Dungeon.isChallenged(Challenges.SWARM_INTELLIGENCE)) {
				...
				for (Mob mob : Dungeon.level.mobs) {
					if (mob.paralysed <= 0
							...
							&& mob.state != mob.HUNTING && !(hero != null && hero.hasTalent(Talent.NO_MORE_MOB) && Random.Int(2)==1)) {
						if(hero != null && hero.pointsInTalent(Talent.NO_MORE_MOB)>1){
							state = WANDERING;
						}else{
							mob.beckon(target);
						}
		...
		protected boolean handleRecentAttackers(){
			...
			if (!recentlyAttackedBy.isEmpty()){
				for (Char ch : recentlyAttackedBy){
					if (ch != null && ch.isActive() && Actor.chars().contains(ch) && Actor.isHostile(Mob.this, ch) && fieldOfView[ch.pos] && ch.invisible == 0 && !isCharmedBy(ch)) {
