...
public class Buff extends Actor {
    ...
    public static<T extends FlavourBuff> T extend( Char target, Class<T> buffClass, float duration ) {
        float staticDuration;
        try {
            Field field = buffClass.getField("DURATION");
            staticDuration = field.getFloat(null); // null 表示取静态属性
        } catch (NoSuchFieldException e) {
            throw new RuntimeException("Failed to get static DURATION from " + buffClass, e);
        } catch (IllegalAccessException e) {
            throw new RuntimeException("Failed to get static DURATION from " + buffClass, e);
        }
        return extend(target, buffClass,duration, staticDuration);
    }
    public static<T extends FlavourBuff> T extend( Char target, Class<T> buffClass, float duration, float Duration) {
