...
public class Hero extends Char {
	...
	public void updateHT( boolean boostHP ){
		...
		//史莱姆
		if (this.subClass.is(HeroSubClass.WATERSLIME)){
			HT += (int) Math.round(0.5 * lvl * (1 + this.pointsInTalent(Talent.WATER_BODY)));
		}


		float multiplier = RingOfMight.HTMultiplier(this);
		HT = Math.round(multiplier * HT);
		if(hasTalent(Talent.SAVAGE_PHYSIQUE)){
			HT+=5+5*pointsInTalent(Talent.SAVAGE_PHYSIQUE);
		}

		if(pointsNegative(Talent.JIULONGLA_COFFIN)>0){
			HT-=(int)(HT*0.2*pointsNegative(Talent.JIULONGLA_COFFIN));
		}
		
		if (buff(ElixirOfMight.HTBoost.class) != null){
			HT += buff(ElixirOfMight.HTBoost.class).boost();
		}
		if(pointsNegative(Talent.DEEP_FEAR)>0){
			HT-=Statistics.deepestFloor*(pointsNegative(Talent.DEEP_FEAR)+1);
		}
		//史莱姆
		if (this.subClass.is(HeroSubClass.DARKSLIME)){
			HT *= 0.8f;
		}
		if(HT<1){
			HT=1;
	...
	private static final ArrayList<Talent> RT_T1_1 = new ArrayList<>(Arrays.asList(Talent.HEARTY_MEAL, Talent.EMPOWERING_MEAL, Talent.CACHED_RATIONS, Talent.NATURES_BOUNTY, Talent.STRENGTHENING_MEAL));
	private static final ArrayList<Talent> RT_T1_2 = new ArrayList<>(Arrays.asList(Talent.VETERANS_INTUITION, Talent.SCHOLARS_INTUITION, Talent.THIEFS_INTUITION, Talent.SURVIVALISTS_INTUITION, Talent.ADVENTURERS_INTUITION));
	private static final ArrayList<Talent> RT_T1_3 = new ArrayList<>(Arrays.asList(Talent.PROVOKED_ANGER, Talent.LINGERING_MAGIC, Talent.SUCKER_PUNCH, Talent.FOLLOWUP_STRIKE, Talent.PATIENT_STRIKE));
	private static final ArrayList<Talent> RT_T1_4 = new ArrayList<>(Arrays.asList(Talent.IRON_WILL, Talent.BACKUP_BARRIER, Talent.PROTECTIVE_SHADOWS, Talent.NATURES_AID, Talent.AGGRESSIVE_BARRIER));

	private static final ArrayList<Talent> RT_T2_1 = new ArrayList<>(Arrays.asList(Talent.IRON_STOMACH, Talent.ENERGIZING_MEAL, Talent.MYSTICAL_MEAL, Talent.INVIGORATING_MEAL, Talent.FOCUSED_MEAL));
	private static final ArrayList<Talent> RT_T2_2 = new ArrayList<>(Arrays.asList(Talent.IMPROVISED_PROJECTILES, Talent.ARCANE_VISION, Talent.WIDE_SEARCH, Talent.HEIGHTENED_SENSES));
	private static final ArrayList<Talent> RT_T2_3 = new ArrayList<>(Arrays.asList(Talent.LIQUID_WILLPOWER, Talent.INSCRIBED_POWER, Talent.INSCRIBED_STEALTH, Talent.LIQUID_NATURE, Talent.LIQUID_AGILITY));
	private static final ArrayList<Talent> RT_T2_4 = new ArrayList<>(Arrays.asList(Talent.RUNIC_TRANSFERENCE, Talent.WAND_PRESERVATION, Talent.ROGUES_FORESIGHT, Talent.REJUVENATING_STEPS));
	private static final ArrayList<Talent> RT_T2_5 = new ArrayList<>(Arrays.asList(Talent.LETHAL_MOMENTUM, Talent.SILENT_STEPS, Talent.DURABLE_PROJECTILES));
	private static final ArrayList<Talent> RT_T3_1 = new ArrayList<>(Arrays.asList(Talent.ENDLESS_RAGE, Talent.DEATHLESS_FURY, Talent.ENRAGED_CATALYST));
	private static final ArrayList<Talent> RT_T3_2 = new ArrayList<>(Arrays.asList(Talent.CLEAVE, Talent.LETHAL_DEFENSE, Talent.ENHANCED_COMBO));
	private static final ArrayList<Talent> RT_T3_3 = new ArrayList<>(Arrays.asList(Talent.EMPOWERED_STRIKE, Talent.MYSTICAL_CHARGE, Talent.EXCESS_CHARGE));
	private static final ArrayList<Talent> RT_T3_4 = new ArrayList<>(Arrays.asList(Talent.SOUL_EATER, Talent.SOUL_SIPHON, Talent.NECROMANCERS_MINIONS));
	private static final ArrayList<Talent> RT_T3_5 = new ArrayList<>(Arrays.asList(Talent.ENHANCED_LETHALITY, Talent.ASSASSINS_REACH, Talent.BOUNTY_HUNTER));
	private static final ArrayList<Talent> RT_T3_6 = new ArrayList<>(Arrays.asList(Talent.EVASIVE_ARMOR, Talent.PROJECTILE_MOMENTUM, Talent.SPEEDY_STEALTH));
	...
	public int pointsInTalent( Talent talent ){
		...
		if (heroClass == HeroClass.RATKING){
			...
			if(RT_T1_2.contains(talent)) return pointsInTalent(Talent.ROYAL_INTUITION);
			if(RT_T1_3.contains(talent)) return pointsInTalent(Talent.KINGS_WISDOM);
			if(RT_T1_4.contains(talent)) return pointsInTalent(Talent.NOBLE_CAUSE);

			if(RT_T2_1.contains(talent)) return pointsInTalent(Talent.RK_ROYAL_MEAL);
			if(RT_T2_2.contains(talent)) return pointsInTalent(Talent.KINGS_VISION);
			if(RT_T2_3.contains(talent)) return pointsInTalent(Talent.RESTORATION);
			if(RT_T2_4.contains(talent)) return pointsInTalent(Talent.POWER_WITHIN);
			if(RT_T2_5.contains(talent)) return pointsInTalent(Talent.PURSUIT);

			if(RT_T3_1.contains(talent)) return pointsInTalent(Talent.RK_BERSERKER);
			if(RT_T3_2.contains(talent)) return pointsInTalent(Talent.RK_GLADIATOR);
			if(RT_T3_3.contains(talent)) return pointsInTalent(Talent.RK_BATTLEMAGE);
			if(RT_T3_4.contains(talent)) return pointsInTalent(Talent.RK_WARLOCK);
			if(RT_T3_5.contains(talent)) return pointsInTalent(Talent.RK_ASSASSIN);
			if(RT_T3_6.contains(talent)) return pointsInTalent(Talent.RK_FREERUNNER);
			if(RT_T3_7.contains(talent)) return pointsInTalent(Talent.RK_SNIPER);
			if(RT_T3_8.contains(talent)) return pointsInTalent(Talent.RK_WARDEN);
		}
	...
	public int pointsNegative( Talent talent ){
		for (LinkedHashMap<Talent, Integer> tier : talents){
			for (Talent f : tier.keySet()){
                int talentlvl = tier.get(f);
                if(f == talent && Dungeon.isChallenged(Challenges.HARSH_ENVIRONMENT)){
                    talentlvl/=2;
                }
				if (f == talent) return 2-talentlvl;
			}
		}
		return 0;
	}
	...
	public int talentPointsAvailable(int tier){
		int avalvl=lvl;
		if(Dungeon.isChallenged(Challenges.WEAKENED_TALENT)){
			if(Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT))avalvl=3*((lvl-1)/3)+1;
			else if(Dungeon.isChallenged(Challenges.HARSH_ENVIRONMENT))avalvl=2*((lvl-1)/2)+1;
		}
		if (avalvl < (Talent.tierLevelThresholds[tier] - 1)
			|| (tier == 3 && subClass.is(HeroSubClass.NONE) && Dungeon.hero.heroClass!= HeroClass.FREEMAN)
			|| (tier == 4 && armorAbility == null)) {
			return 0;
		} else if (avalvl >= Talent.tierLevelThresholds[tier+1]){
			return Talent.tierLevelThresholds[tier+1] - Talent.tierLevelThresholds[tier] - talentPointsSpent(tier) + bonusTalentPoints(tier);
		} else {
			return 1 + avalvl - Talent.tierLevelThresholds[tier] - talentPointsSpent(tier) + bonusTalentPoints(tier);
		}
	}
	...
	public int bonusTalentPoints(int tier){
		...
		if (lvl < (Talent.tierLevelThresholds[tier]-1)
				|| (tier == 3 && subClass.is(HeroSubClass.NONE))
				|| (tier == 4 && armorAbility == null)) {
			bonus+=0;
		}
		...
		if (buff(ScrollOfSublimation.Sublimation.class) != null
				&& buff(ScrollOfSublimation.Sublimation.class).isBoosted(tier)) {
			bonus+=1;
		}
		if (buff(ScrollOfSublimation.Sublimation1.class) != null
				&& buff(ScrollOfSublimation.Sublimation1.class).isBoosted(tier)) {
			bonus+=1;
		}
		return bonus;
	}
	...
	public String className() {
		return subClass == null || subClass.is(HeroSubClass.NONE) ? heroClass.title() : subClass.title();
	}
	...
	public void chooseEnemy(Char c){
		this.enemy = c;
	}
	...
	public int attackSkill( Char target ) {
		...
		float accuracy = 1;
		if (pointsInTalent(Talent.WOLFISH_GAZE)==2){
			return INFINITE_ACCURACY;
		}else if(pointsInTalent(Talent.WOLFISH_GAZE)==1){
			accuracy *= 4;
		}
		if(pointsNegative(Talent.PHOTOPHOBY)>0 && !buffs(Light.class).isEmpty()){
			accuracy*=1-0.2*hero.pointsNegative(Talent.PHOTOPHOBY);
		}
		if(pointsInTalent(Talent.PRECISE_SHOT)>0 && wep instanceof MissileWeapon && Dungeon.level.distance(pos,target.pos)>=3){
			accuracy*=1+0.1f*hero.pointsInTalent(Talent.PRECISE_SHOT);
		}

		if(!target.buffs(PinCushion.class).isEmpty() && pointsInTalent(Talent.INSERT_BID)>=1 ){
			accuracy *= 1+pointsInTalent(Talent.INSERT_BID)*0.2;
		}
		if(attackDelay()>1 && pointsInTalent(Talent.OVERWHELMING)>=1){
		...
		if(pointsNegative(Talent.MYOPIA)>0){
			accuracy*=1-pointsNegative(Talent.MYOPIA)*0.2;
		}
        ...
        if(fightStance!=null){
            if(fightStance.stance==fightStance.balance){
                accuracy*=1.1f+Math.min(1,pointsInTalent(Talent.STANCE_MASTERY))*0.05f;
            }
            if(fightStance.stance==fightStance.parry){
                accuracy*=0.7f+pointsInTalent(Talent.STANCE_MASTERY)/3*0.15f;
            }
        }
        if(glyphLevel(Stone.class)>=0 && subClass.is(HeroSubClass.COMBATMASTER)){
            accuracy+=defenseSkill(target);
        }
		...
		if (!(wep instanceof MissileWeapon)) {
			if ((hasTalent(Talent.PRECISE_ASSAULT) || hasTalent(Talent.LIQUID_AGILITY))
				...
				if (wep instanceof Flail && buff(Flail.SpinAbilityTracker.class) != null){
					...
					//do nothing, this is not a regular attack so don't consume talent fx
				} else if (wep instanceof Blowpipe && buff(Blowpipe.WeakChargedShot.class) != null){
					//Blowpipe
					//do nothing, this is not a regular attack so don't consume talent fx
	...
	public int defenseSkill( Char enemy ) {
		...
		if (buff(RoundShield.GuardTracker.class) != null || buff(Talent.PhantomStep.class)!=null){
			return INFINITE_EVASION;
		}
		
		float evasion = defenseSkill;
		if(pointsInTalent(Talent.LIGHT_APPLICATION)>0 && belongings.armor() != null){
			evasion+=(5-belongings.armor().tier)*pointsInTalent(Talent.LIGHT_APPLICATION);
		}
        ...
        if(glyphLevel(Flow.class)>=0 && subClass.is(HeroSubClass.COMBATMASTER) && Dungeon.level.water[pos]){
            evasion+=lvl;
        }
        ...
        if(fightStance!=null){
            if(fightStance.stance==fightStance.balance){
                evasion*=1.1f+Math.min(1,pointsInTalent(Talent.STANCE_MASTERY))*0.05f;
            }else if(fightStance.stance==fightStance.invasion){
                evasion*=0.7f+pointsInTalent(Talent.STANCE_MASTERY)/3*0.15f;
            }
        }
		...
		if (belongings.armor() != null) {
			...
			if (ring != null && ring.isEquipped(hero) && ring.glyph != null && !ring.cursed && ring.glyph.getClass() == Stone.class){
				evasion = 0;
			}
		}

		if (buff(Suffering.Ecstasy.class) != null){
			evasion *= 0.5f;
		}
		if (buff(HolyPrayer.HolyPrayerBlessing.class) != null){
	...
	public int defenseSkill2( Char enemy ) {
	...
	public String defenseVerb() {
        ...
        if (buff(FightStance.Focus.class) != null){
            buff(FightStance.Focus.class).detach();
	...
	public int drRoll() {
		...
		if (belongings.armor() != null) {
			...
			if (spsh != null) {
				//GLog.i("护甲护甲("+belongings.armor().DRMin()+"-"+belongings.armor().DRMax()+")-->("+spsh.changeDrMin(belongings.armor().DRMin(), belongings.armor().DRMax())+"-"+spsh.changeDrMax(belongings.armor().DRMin(), belongings.armor().DRMax())+")");
				armDr = Random.NormalIntRange(spsh.changeDrMin(belongings.armor().DRMin(), belongings.armor().DRMax()),
											  spsh.changeDrMax(belongings.armor().DRMin(), belongings.armor().DRMax()));
			}
		...
		if(buff(Talent.ResilientArmor.class) != null){
			dr += Random.Int(4);
		}
		if(hasTalent(Talent.BREAK_ENEMY_RANKS)){
			int n = 0;
			for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
				if (Dungeon.level.heroFOV[mob.pos] && mob.alignment== Alignment.ENEMY) {
					n++;
				}
			}
			dr += n * (Random.Int(4) + pointsInTalent(Talent.BREAK_ENEMY_RANKS)-1);
		}
		if(subClass.is(HeroSubClass.AU400)){
			ArrayList<InstructionTool.Drone> drones = InstructionTool.getDroneAlly();
			if(drones!=null){
				int num_drones=0;
				for(InstructionTool.Drone d:drones){
					if(d instanceof AuxiliaryDrone.ProtectDrone){
						num_drones++;
					}
				}
        ...
        if(fightStance!=null){
            if(fightStance.stance==fightStance.balance){
                dr*=1.1f+Math.min(1,pointsInTalent(Talent.STANCE_MASTERY))*0.05f;
            }else if(fightStance.stance==fightStance.parry){
                dr*=1.2f+hero.pointsInTalent(Talent.STANCE_MASTERY)/2*0.1f;
            }
        }
		if(hero.pointsNegative(Talent.SHELL_CRY)>0){
			dr = (int)(dr*(1-((float)(HT-HP)/HT/2)*hero.pointsNegative(Talent.SHELL_CRY)));
		}
	...
	public int damageRoll() {
		...
		if (!RingOfForce.fightingUnarmed(this)) {
			...
			if(hasTalent(Talent.SHADOW_KILLER) && enemy instanceof Mob && ((Mob) enemy).surprisedBy(hero)){
				int min=100000000;
				int max=0;
				for(int i =0;i<1000;i++){
					int d=RingOfForce.damageRoll(this);
					if(d>max){
						max=d;
					}
					if(d<min){
				...
				float mult = 0.5f+0.25f*pointsInTalent(Talent.SHADOW_KILLER);
				//deals 75% toward max to max on surprise, instead of min to max.
				int diff = max - min;
				dmg=min+ Math.round(diff*mult)+(int)(enemy.drRoll()*(0.5f+0.5f*pointsInTalent(Talent.SHADOW_KILLER)));
		...
		if(hasTalent(Talent.SHADOW_KILLER) && wep!=null && !RingOfForce.fightingUnarmed(this)){
			if (enemy instanceof Mob && ((Mob) enemy).surprisedBy(hero)) {
				float mult = 0.5f+0.25f*pointsInTalent(Talent.SHADOW_KILLER);
				//deals 75% toward max to max on surprise, instead of min to max.
				int diff = wep.max() - wep.min();
				int damage = ((Weapon)belongings.attackingWeapon()).augment.damageFactor(Hero.heroDamageIntRange(
						wep.min() + Math.round(diff*mult),
						wep.max()));
				int exStr = hero.STR() - ((Weapon)belongings.attackingWeapon()).STRReq();
				if (exStr > 0) {
					damage += heroDamageIntRange(0, exStr);
				}
				int ringdmg=0;
				if (hero.buff(RingOfForce.BrawlersStance.class) != null
					...
					//deals 75% toward max to max on surprise, instead of min to max.
					diff = max - min;
					ringdmg=min+ Math.round(diff*mult)+(int)(enemy.drRoll()*(0.5f+0.5f*pointsInTalent(Talent.SHADOW_KILLER)));
				}else{
					if (!(wep instanceof MissileWeapon)) ringdmg = RingOfForce.armedDamageBonus(this);
				}
	...
	public float speed() {
		...
		if (this.pointsInTalent(Talent.RUSH)==1){
			speed*=1.5;
		}else if(this.pointsInTalent(Talent.RUSH)==2){
			speed*=1.75;
		}
		if (this.pointsInTalent(Talent.OVERLOAD_CHARGE)==1 && (this.buff(Recharging.class)!=null || this.buff(ArtifactRecharge.class)!=null)){
			speed*=1.5;
		}else if(this.pointsInTalent(Talent.OVERLOAD_CHARGE)==2 && (this.buff(Recharging.class)!=null || this.buff(ArtifactRecharge.class)!=null)){
			speed*=1.75;
		}
		if(HP<HT*0.3 && hasTalent(Talent.AFRAID_DEATH)){
			speed*=1.2+pointsInTalent(Talent.AFRAID_DEATH)*0.1;
		}
		if(belongings.getItem( Amulet.class)!=null && hasTalent(Talent.HOMETOWN_CLOUD)){
			speed*=1+0.2*pointsInTalent(Talent.HOMETOWN_CLOUD);
		}
		...
		if(hasTalent(Talent.SEAOFPEOPLE)){
			...
			speed *= 1+(0.15f+0.15f*pointsInTalent(Talent.SEAOFPEOPLE))*n;
		}
        FightStance fightStance = buff(FightStance.class);
        if(fightStance!=null){
	...
	public float attackDelay() {
        ...
        if(buff(MarchForward.Forward.class)!=null && 3*pointsInTalent(Talent.RUN_ATTACK)>Random.Int(40)){
            return 0;
        }
	...
	public boolean act() {
		...
		if(Dungeon.level.water[this.pos] && this.buffs(Talent.AquaticRecover.class).isEmpty() && this.hasTalent(Talent.AQUATIC_RECOVER)){
			Buff.affect(this,Talent.AquaticRecover.class);
		...
		if (curAction == null) {
			...
			if(!buffs(Talent.NoSleep.class).isEmpty()){
				buff(Talent.NoSleep.class).detach();
			}
	...
	private boolean actMove( HeroAction.Move action ) {
		if(pointsInTalent(Talent.EXPLORATION_INTUITION)>Random.Int(2)){
			search(false);
		}
					...
					public void call() {
						...
						//1 hunger spent total
							...
							Level.set( action.dst, Terrain.EMPTY_DECO );
								...
								|| Dungeon.level.map[action.dst] == Terrain.REGION_DECO_ALT){
							Splash.at(action.dst, 0x555555, 10);
							Sample.INSTANCE.play( Assets.Sounds.MINE, 0.6f );
                            if (Dungeon.depth < 6 && Dungeon.depth > 0){
                                Level.set( action.dst, Terrain.WATER );
                            }else if(Dungeon.depth < 11 && Dungeon.depth > 5 && Dungeon.level.map[action.dst] == Terrain.REGION_DECO_ALT){
                                Level.set( action.dst, Terrain.CHASM );
                            }else{
                                Level.set( action.dst, Terrain.EMPTY_DECO );
                                Dungeon.level.removeWallVisuals(CityLevel.GreenFlame.class, action.dst);
                            }
	...
	private boolean actAttack( HeroAction.Attack action ) {
		...
		enemy = action.target;
		if(Dungeon.isChallenged(Challenges.NO_FOOD) && Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT)){
			buff(Hunger.class).affectHunger(-0.2f);
		}
		...
		if (enemy.isAlive() && canAttack( enemy ) && enemy.invisible == 0) {
			...
			if (!MeleeWeapon.canUseWeaponAbility(this)
					...
					&& (HP / (float)HT) < 0.50f){
				int heal = 1+2*pointsInTalent(Talent.AGGRESSIVE_BARRIER);
				Buff.affect(this, Barrier.class).setShield(heal);
				sprite.showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(heal), FloatingText.SHIELDING);
	...
	public void rest( boolean fullRest ) {
		...
		if(pointsNegative(Talent.LIFE_SPORT)>0){
			...
			if (!this.isAlive()) {
				Dungeon.fail( Talent.LIFE_SPORT );
				GLog.n( Messages.get(Talent.LIFE_SPORT, Talent.LIFE_SPORT.name()+".kill") );
			}
		}
		...
		if(Dungeon.isChallenged(Challenges.NO_FOOD) && Dungeon.isChallenged(Challenges.EXTREME_ENVIRONMENT)){
			if(buff(Hunger.class)!=null){buff(Hunger.class).affectHunger(-0.2f);}
		}
		if (!fullRest) {
			if (sprite != null) {
	...
	public int attackProc( final Char enemy, int damage , DamageTag... damageTags) {
		...
		if (wep != null) {
            ...
            Weapon.Enchantment ringEnchant = null;
            RingOfKing ring = hero.belongings.getItem(RingOfKing.class);
            if (ring != null && ring.isEquipped(hero) && ring.enchantment != null && !ring.cursed){
                ringEnchant = ring.enchantment;
                damage = ringEnchant.proc(ring.getWeapon(), this, enemy, damage);
            }
            if (hero.subClass.is(HeroSubClass.RUNEMAGE) && enemy!=this){
                RuneMark runeMark = enemy.buff(RuneMark.class);
                if(runeMark==null ){
                    runeMark = Buff.affect(enemy, RuneMark.class);
                }

                if(ringEnchant!=null){
                    runeMark.addEnchantment(ringEnchant);
                }
                if(runeMark.enchantments.isEmpty()){
                    runeMark.detach();
                }else{
                    if(hero.pointsInTalent(Talent.DARKMARK)>2){
                        Buff.affect(enemy, Weakness.class,3);
                    }
                    if(hero.pointsInTalent(Talent.DARKMARK)>1){
                        Buff.affect(enemy, Vulnerable.class,3);
                    }
                    if(hero.pointsInTalent(Talent.DARKMARK)>0){
                        Buff.affect(enemy, Hex.class,3);
                    }
                }
            }
                    ...
                    protected boolean act() {
                        if (enemy.isAlive()) {
                            if (hasTalent(Talent.SHARED_UPGRADES)) {
                                int bonusTurns = wep.buffedLvl() * pointsInTalent(Talent.SHARED_UPGRADES);
	...
	public int defenseProc( Char enemy, int damage , DamageTag... damageTags) {
		...
		if (belongings.armor() != null) {
            ...
            Armor.Glyph ringGlyph = null;
            RingOfKing ring = hero.belongings.getItem(RingOfKing.class);
            if (ring != null && ring.glyph != null && !ring.cursed){
                ringGlyph = ring.glyph;
                damage = ringGlyph.proc( ring.getArmor(), enemy, this, damage );
            }
	...
	public int glyphLevel(Class<? extends Armor.Glyph> cls) {
        ...
        if (ring != null && ring.isEquipped(this) && ring.glyph != null && !ring.cursed && ring.glyph.getClass() == cls){
            lvl = ring.truelevel();
            //GLog.i(String.valueOf(lvl));
        }
		if (belongings.armor() != null && belongings.armor().glyph != null && belongings.armor().glyph.getClass() == cls){
            if(lvl==-1){
                lvl = 0;
            }
			return lvl+Math.max(super.glyphLevel(cls), belongings.armor.buffedLvl());
		} else if (buff(BodyForm.BodyFormBuff.class) != null
				&& buff(BodyForm.BodyFormBuff.class).glyph() != null
				&& buff(BodyForm.BodyFormBuff.class).glyph().getClass() == cls){
			return lvl+(belongings.armor() != null ? belongings.armor.buffedLvl() : 0);
		}else if(lvl>-1){
            return lvl;
        } else {
	...
	private boolean getCloser( final int target ) {
		...
		if (step != -1) {
			...
			if(pointsNegative(Talent.WINTER_SWIMMING)>Random.Int(20) && Dungeon.level.water[pos]){
				Buff.affect(this, Chill.class,3);
			}
	...
	public void earnExp( int exp, Class source ) {
		...
		Shuriken_Box.shurikenRecharge box = buff(Shuriken_Box.shurikenRecharge.class);
		if (box != null) box.gainExp(percent);
		...
		//Spsh3
		SpearShield.SpearShieldCooldown sscd = buff(SpearShield.SpearShieldCooldown.class);
		SpearShield spsh = Dungeon.hero.belongings.getItem(SpearShield.class);
		if (sscd != null && spsh != null) {
			sscd.recover(percent);
		}
	...
	public boolean upLevel(int level) {
		...
		if (lvl+level <= MAX_LEVEL || level<0) {
			lvl+=level;
			...
			attackSkill+=level;
			defenseSkill+=level;
			if(hasTalent(Talent.HERO_NAME) && level>0){
                heal(HT*pointsInTalent(Talent.HERO_NAME)/5);
			}

			if(hasTalent(Talent.ACTIVE_MUSCLES) && level>0){
	...
	public void onAttackComplete() {
        ...
        FightStance fightStance = buff(FightStance.class);
        if(fightStance!=null){
            if(fightStance.stance==fightStance.invasion && 3+2*hero.pointsInTalent(Talent.STANCE_MASTERY)/3>Random.Int(10)
                    && wasEnemy && enemy!=null && enemy.isAlive()){
	...
	public void resurrect() {
		...
		for (Item i : belongings){
			...
			}else if(i instanceof RitualDagger && i.keptThroughLostInventory()){
                ritual=true;
            }
		}
        if(buff(RitualDagger.RitualTracker.class)!=null && !ritual){
            buff(RitualDagger.RitualTracker.class).detach();
        }
