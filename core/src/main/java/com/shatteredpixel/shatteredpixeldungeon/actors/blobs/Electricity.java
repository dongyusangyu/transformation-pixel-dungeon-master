			...
			for (int j = area.top-1; j <= area.bottom; j++) {
				...
				if (cur[cell] > 0) {
					...
					if (ch != null && (!ch.isImmune(this.getClass()) && !(ch instanceof GoldBoss))) {
						...
						if (cur[cell] % 2 == 1) {
							ch.damage(Math.round(Random.Float(2 + Dungeon.scalingDepth() / 5f)), this, DamageTag.PHYSICAL, DamageTag.ELECTRIC);
					...
					if(ch instanceof GoldBoss){
						Buff.affect(ch, Haste.class,3);
					}
