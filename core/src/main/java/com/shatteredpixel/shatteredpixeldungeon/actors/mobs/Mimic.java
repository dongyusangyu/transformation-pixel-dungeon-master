...
public class Mimic extends Mob {
	...
	public float speed() {
		...
		if(hero.pointsNegative(Talent.FULLPASSION)>0){
			speed*=1+0.5f*hero.pointsNegative(Talent.FULLPASSION);
		}
	...
	public boolean interact(Char c) {
		if (alignment != Alignment.NEUTRAL || c != hero){
	...
	public int damageRoll() {
		if (alignment == Alignment.NEUTRAL){
			...
			if(hero.pointsNegative(Talent.FULLPASSION)>0){
				damage=(int)(damage*(1+0.3*hero.pointsNegative(Talent.FULLPASSION)));
			}
			return damage;
		} else {
	...
	public static Mimic spawnAt( int pos, Class mimicType, boolean useDecks, Item... items){
		...
		if (mimicType == GoldenMimic.class){
			...
			m = new EbonyMimic();
		}else if (mimicType == TransformMimic.class) {
			m = new TransformMimic();
		} else {
			m = new Mimic();
		}
