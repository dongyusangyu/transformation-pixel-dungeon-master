...
public class Sungrass extends Plant {
	...
	public void activate( Char ch ) {
		...
		if (ch != null){
			...
			if (ch instanceof Hero && ((Hero) ch).subClass == HeroSubClass.WARDEN) {
				...
				//史莱姆
				if(((Hero) ch).hasTalent(Talent.ORIGINAL_MONSTER)){
					Healing = (int)(Healing * 1.5);
				}
				Buff.affect(ch, Health.class).boost(Healing);
			} else {
	...
	public static class Health extends Buff {
		...
		public boolean act() {
			...
			if(hero != null && hero.hasTalent(Talent.EFFICIENT_HEALING)){
				partialHeal += 1+0.5f*hero.pointsInTalent(Talent.EFFICIENT_HEALING);
			}else{
