...
public abstract class Plant implements Bundlable {
	...
	public void wither() {
		...
		if(Dungeon.hero.pointsInTalent(Talent.SEED_RECYCLING)>Random.Int(5)){
	...
	public String desc() {
		...
		if (Dungeon.hero != null && Dungeon.hero.subClass.is(HeroSubClass.WARDEN)){
	...
	public static class Seed extends Item {
		...
		//史莱姆
		public static final String AC_PLANT_IN_BODY	= "PLANT_IN_BODY";
		public static final String AC_EAT	= "eat";
		
		private static final float TIME_TO_PLANT = 1f;
		private static final float TIME_TO_PLANT_IN_BODY = 1f;
		...
		public ArrayList<String> actions( Hero hero ) {
			...
			if(hero.heroClass == HeroClass.SLIMEGIRL){
				actions.add( AC_PLANT_IN_BODY );
			}
			if(hero.heroClass !=HeroClass.SLIMEGIRL && hero.hasTalent(Talent.SLIME_GREENHOUSE)){
				actions.add( AC_EAT );
			}
			return actions;
		}
		...
		public void execute( Hero hero, String action ) {
			...
			}else if (action.equals( AC_PLANT_IN_BODY )) {//史莱姆

				hero.busy();
				detach( hero.belongings.backpack );

				if(!hero.hasTalent(Talent.SLIME_GREENHOUSE)){
					...
					if(Dungeon.isChallenged(Challenges.NO_HERBALISM)){
						hero.spend( TIME_TO_PLANT_IN_BODY*15 );
					}
				}
